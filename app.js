(() => {
  ...
  const KEY_DATES = {
    ...
    // Визит к трихологу: 2 месяца от старта полного приёма (8 июня)
    fullRegimenStart:  mkd("2026-06-08"), // когда подключены почти все таблетки
  ...
  const DEFAULT_PILLS = [
    ...
    { id: "forlax", name: "Форлакс 1 пакет", time: "15:00", color: PC.LUNCH, bg: PC.LUNCH_BG,
      note: "Осмотическое слабительное (макрогол). Развести в стакане воды, с едой или без. ОТДЕЛЬНО от других таблеток — ±2 ч (ускоряет транзит, мешает всасыванию). Работает мягко через 24–48 ч. План: один до ~24 июня → затем вместе с Фитомуцилом → с 15 июля убираем, остаётся Фитомуцил. Старт 30 мая.",
      startDate: "2026-05-30", endDate: "2026-07-15" },
    { id: "phytomucil", name: "Фитомуцил Норм Плюс 1 пакетик", time: "17:30", color: PC.LUNCH, bg: PC.LUNCH_BG,
      note: "Растворимая клетчатка (псиллиум + слива). Развести/выпить и ОБЯЗАТЕЛЬНО дополнительно запить водой (иначе наоборот крепит). Вводим с 24 июня — сначала вместе с Форлаксом, потом вместо него, длительно. При необходимости до 2 пакетиков/день по инструкции.",
  ...
  const INTAKE_RULES = {
    ...
    forlax: { can: ["Развести в стакане воды", "С едой или без — на еду не завязан", "Пить больше воды днём"], cant: ["Другие таблетки ±2 ч (ускоряет кишечник, мешает всасыванию)"], gapBefore: 2, gapAfter: 2, gapReason: "другие таблетки" },
    ...
    phytomucil: { can: ["В 17:30–18:00, развести", "ОБЯЗАТЕЛЬНО дополнительно запить водой"], cant: ["Без достаточной воды — наоборот крепит"] },
  ...
  const PLAN_BLOCKS = [
    {
      ...
      sections: [
        ...
        { h: "Купить до 2 июня", items: [
          ...
          "A+E Medana, Цистениум, Кристалваг (старт 8 июня); Лактриол с 13 июня"
        ]},
    ...
    },
    {
      ...
      sections: [
        { h: "Что подключается", items: [
          "ПН 8 июня — вечер: A+E Medana 2 капс; на ночь: Цистениум + Кристалваг 2 капс",

  ...
  function TodayTab({ todayDow, DR, setTab, workoutDays, cycleAnchor, packAnchor, periodOverrides, badDay, userName }) {
    ...
    return React.createElement("div", null,
      ...
      diaryDone && !showDiary && (() => {
        ...
        const energyLabels = ["", "нет сил", "мало", "ок", "хорошо", "много"];
        const parts = [];
        if (d.mood) parts.push(moodEmojis[d.mood] || "");
        if (d.energy) parts.push("энергия: " + (energyLabels[d.energy] || d.energy));
        if (d.gut) parts.push("ЖКТ: " + d.gut);
        return React.createElement("div", {
          style: { background: C.okSoft || (C.ok + "14"), border: `1px solid ${C.ok}55`, borderRadius: 12, padding: "12px 14px", marginBottom: 10 }
        },
          React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
              React.createElement("div", { style: { width: 22, height: 22, borderRadius: "50%", background: C.ok, color: "#fff", fontSize: 13, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 } }, "✓"),
              parts.length > 0 && parts[0] && /😞|😐|🙂|😊|😄/.test(parts[0])
                ? React.createElement("span", { style: { fontSize: 20 } }, parts[0])
                : null,
              React.createElement("div", { style: { fontSize: 13, color: C.ok, fontWeight: 700 } }, "Дневник заполнен")
            ),
            React.createElement("button", { onClick: () => setShowDiary(true),
              style: { padding: "6px 14px", borderRadius: 8, background: C.card, border: `0.5px solid ${C.ok}55`, fontSize: 12, color: C.text, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }
            }, "Изменить")
          ),
          (() => {
            const rest = parts.filter((p, i) => !(i === 0 && /😞|😐|🙂|😊|😄/.test(p)));
            return rest.length > 0 && React.createElement("div", { style: { fontSize: 12, color: C.textM, marginTop: 7, lineHeight: 1.5 } }, rest.join(" · "));
          })(),
          d.note && React.createElement("div", { style: { fontSize: 11.5, color: C.textM, marginTop: 4, fontStyle: "italic", lineHeight: 1.45 } }, "«", d.note, "»")
        );
      })(),

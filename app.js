(() => {
  ...
  function DayScheduleCard({ cycleAnchor, periodOverrides, workoutDays, packAnchor, onOpen }) {
    ...
    // Собираем расписание дня (фильтрованное по активным препаратам)
    const items = [];
    items.push({ time: "08:00", label: "Подъём, стакан воды", icon: "☀️", kind: "wake" });
    if (isIronDay(today)) {
      items.push({ time: "08:15", label: "Железо + витамин C (натощак, с водой!)", icon: "💊", kind: "pill_iron" });
    }
    items.push({ time: "09:00", label: "Дуксет 60 мг (натощак)", icon: "💊", kind: "pill" });
    items.push({ time: "09:30", label: "Завтрак", icon: "🍳", kind: "meal" });

    // Обед — жирорастворимое
    const lunchPills = [];
    if (today >= KEY_DATES.omegaStart) lunchPills.push("Омега 2 капс");
    if (today >= KEY_DATES.vitDStart && (dow === 0 || dow === 2 || dow === 5)) lunchPills.push("Витамин D");
    if (today >= KEY_DATES.perfectilStart) lunchPills.push("Перфектил");
    items.push({ time: "14:00", label: lunchPills.length ? `Обед + ${lunchPills.join(", ")}` : "Обед", icon: "🥗", kind: "meal" });

    if (today >= KEY_DATES.forlaxStart) {
      items.push({ time: "15:00", label: "Форлакс (отдельно от таблеток)", icon: "💊", kind: "pill" });
    }
    ...
    // Вечерний блок с ужином
    const dinPills = [];
    if (today >= KEY_DATES.zincStart) dinPills.push("Zinkorot 25 мг");
    if (today >= KEY_DATES.niacinStart) dinPills.push("Ниацинамид");
    if (today >= KEY_DATES.aeStart) dinPills.push("A+E Medana 2 капс");
    if (dinPills.length) {
      const dinTime = isTrainingDay ? "19:45" : "20:00";
      items.push({ time: dinTime, label: dinPills.join(", "), icon: "💊", kind: "pill" });
    }
    ...
    // Ярина если сегодня день пачки
    if (packAnchor && isYarinaActiveToday(today, packAnchor)) {
      items.push({ time: "21:00", label: "Ярина", icon: "💊", kind: "pill" });
    }
    // На ночь
    const nightPills = [];
    if (today >= KEY_DATES.cysteniumStart) nightPills.push("Цистениум");
    if (today >= KEY_DATES.biofemaleStart) nightPills.push("Био Фимейл");
    if (nightPills.length) {
      items.push({ time: "22:00", label: nightPills.join(", ") + " (на ночь)", icon: "💊", kind: "pill" });
    }
  ...
  function FullScheduleModal({ onClose, workoutDays, cycleAnchor, packAnchor, periodOverrides }) {
    ...
    if (isIronDay(today)) {
      items.push({ time: "08:15", label: "Железо + витамин C 💊", note: "Натощак, за 30–45 мин до завтрака. Без кофе/чая/молочного 1–2 ч" });
    }
    items.push({ time: "09:00", label: "Дуксет 60 мг 💊", note: "Натощак, в одно время" });
    items.push({ time: "09:30", label: "Завтрак 🍳", note: "Белок 20-25 г" });
    const lunchPills = [];
    if (today >= KEY_DATES.omegaStart) lunchPills.push("Омега 2 капс");
    if (today >= KEY_DATES.vitDStart && (dow === 0 || dow === 2 || dow === 5)) lunchPills.push("Витамин D");
    if (today >= KEY_DATES.perfectilStart) lunchPills.push("Перфектил");
    items.push({ time: "14:00", label: lunchPills.length ? `Обед + ${lunchPills.join(", ")} 💊` : "Обед 🍽", note: "Полноценная еда с жирами, белок 30-35 г" });
    if (today >= KEY_DATES.forlaxStart) {
      items.push({ time: "15:00", label: "Форлакс 💊", note: "Развести в воде. Отдельно от таблеток ±1–2 ч" });
    }
    ...
    if (dinPills.length) {
      const dinTime = isTrainingDay ? "19:45" : "20:00";
      items.push({ time: dinTime, label: dinPills.join(", ") + " 💊", note: "С ужином (с жирами). A+E отдельно от витамина C/селена" });
    }
    ...
    if (packAnchor && isYarinaActiveToday(today, packAnchor)) {
      items.push({ time: "21:00", label: "Ярина 💊", note: "Строго в одно время" });
    }
    const nightPills = [];
    if (today >= KEY_DATES.cysteniumStart) nightPills.push("Цистениум");
    if (today >= KEY_DATES.biofemaleStart) nightPills.push("Био Фимейл");
    if (nightPills.length) {
      items.push({ time: "22:00", label: nightPills.join(", ") + " 💊", note: "На ночь, запивать водой" });
    }
  ...
  function PillsModule({ compact, cycleAnchor, packAnchor }) {
    // Полный набор препаратов по плану 25 мая — 26 июля.
    // Цвета сгруппированы по времени дня: утро (амбер), обед (зелёный),
    // вечер (тёплый коричневый), ночь (приглушённый синий).
    const MORNING = "#BA7517";       // утро (Дуксет, Форлакс)
    const MORNING_BG = "#FAEEDA";
    const PRELUNCH = "#7A4A1A";      // до обеда (Железо)
    const PRELUNCH_BG = "#EDDED0";
    const LUNCH = "#3B6D11";          // обед (Омега, Био, Перфектил, D, Ниацинамид)
    const LUNCH_BG = "#EAF3DE";
    const EVENING = "#7A4A1A";        // вечер (Цинк)
    const EVENING_BG = "#EDDED0";
    const NIGHT = "#534AB7";          // ночь (Ярина, мелатонин)
    const NIGHT_BG = "#EEEDFE";

    // ПОРЯДОК — строго по времени дня, чтобы карточка читалась как таймлайн.
    // СОГЛАСОВАНО (наш план): старт 30 мая (сб). Железо — через день с 2 июня.
    const DEFAULT_PILLS = [
      // 08:15 — Железо натощак, ДО завтрака (через день)
      { id: "iron",       name: "Gentle Iron 25 мг + Витамин C", time: "08:15", color: PRELUNCH, bg: PRELUNCH_BG,
        ...
        ironRule: true /* флаг — особая логика "только в дни железа" */ },
      // 09:00 — Дуксет натощак (просто по времени, не с едой)
      { id: "duxet",      name: "Дуксет 60 мг",         time: "09:00", color: MORNING, bg: MORNING_BG,
      ...
      // 14:00 — Обеденный блок: жирорастворимое + Перфектил
      { id: "perfectil",  name: "Перфектил",            time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "Только после полноценного обеда! Натощак — тошнота. Содержит железо, цинк, B-группу, витамины A/E. С 6 июня.",
        startDate: "2026-06-06" },
      { id: "omega",      name: "Омега-3 (2 капсулы)",  time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "С обедом (с жирами), 2 капсулы. Противовоспалительный эффект, для холестерина. С 6 июня.",
        startDate: "2026-06-06" },
      { id: "vitd",       name: "Витамин D 4000 МЕ",    time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "Только ПН / СР / СБ (3 раза в неделю). С жирной едой — лучше усваивается. С 6 июня.",
        startDate: "2026-06-06",
        weekdays: [0, 2, 5] /* пн, ср, сб (0=пн в моей нумерации) */ },
      // 15:00 — Форлакс ОТДЕЛЬНО от таблеток (±1–2 ч)
      { id: "forlax",     name: "Форлакс 1 пакет",      time: "15:00", color: MORNING, bg: MORNING_BG,
      ...
      // 20:00 — Вечерний блок с ужином: цинк, ниацинамид, A+E
      { id: "zinc",       name: "Zinkorot 25 мг",       time: "20:00", color: EVENING, bg: EVENING_BG,
      ...
      { id: "niac",       name: "Ниацинамид 500 мг",    time: "20:00", color: EVENING, bg: EVENING_BG,
        ...
        startDate: "2026-06-06" },
      { id: "ae",         name: "A+E Medana (2 капсулы)", time: "20:00", color: EVENING, bg: EVENING_BG,
      ...
      // 21:00 — Ярина (по графику пачки)
      { id: "yarina",     name: "Ярина",                time: "21:00", color: NIGHT, bg: NIGHT_BG,
        note: "Строго в 21:00. 21 таблетка + 7 дней перерыв. Приложение само скроет в дни перерыва.",
        yarinaPack: true /* флаг — особая логика "только в дни пачки" */ },
      // 22:00 — На ночь: цистениум, био фимейл, мелатонин (опционально)
      { id: "cystenium",  name: "Цистениум",            time: "22:00", color: NIGHT, bg: NIGHT_BG,
        ...
        startDate: "2026-06-08" },
      { id: "biofemale",  name: "Био Фимейл (пробиотик)", time: "22:00", color: NIGHT, bg: NIGHT_BG,
        note: "Пробиотик для женщин. На ночь, запивать прохладной/тёплой (не горячей) водой. Поддержка микрофлоры. С 8 июня.",
        startDate: "2026-06-08" },
      { id: "mel",        name: "Мелатонин 2 мг (опц.)", time: "22:00", color: NIGHT, bg: NIGHT_BG,
    ...
    // V6 — план синхронизирован с финальной схемой (старт 30 мая, новые времена/даты). Версия поднята для перезагрузки дефолтов.
    const [pills, setPills] = useLS("pillsListV6", DEFAULT_PILLS);
    ...
    const BUILTIN = ["duxet","forlax","omega","biofemale","perfectil","vitd","iron","niac","zinc","ae","yarina","mel","cystenium"];

    // Фильтр: показываем только те препараты, которые сегодня актуальны.
    // - startDate: до этой даты препарат скрыт.
    // - weekdays: показывается только в эти дни недели.
    // - ironRule: показывается только в дни приёма железа.
    // - yarinaPack: показывается только в дни активной пачки Ярины (1..21).
    const todayDate = new Date();
    const todayDow = todayDate.getDay() === 0 ? 6 : todayDate.getDay() - 1;
    const isActive = (p) => {
      if (p.startDate) {
        const start = mkd(p.startDate);
      ...
      if (p.ironRule) {
        if (!isIronDay(todayDate)) return false;
      }
      if (p.yarinaPack && packAnchor) {
        if (!isYarinaActiveToday(todayDate, packAnchor)) return false;
      }
      return true;
    };
            ...
            !BUILTIN.includes(p.id) && (confirmDel === p.id
              ? React.createElement("div", { style: { marginTop: 8, display: "flex", gap: 8, alignItems: "center" } },
                  ...
                  React.createElement("button", { onClick: () => { removePill(p.id); setConfirmDel(null); },
                    style: { background: C.warn, border: "none", color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer", padding: "5px 12px", borderRadius: 7, fontFamily: "inherit" } }, "Да, удалить"),
                  React.createElement("button", { onClick: () => setConfirmDel(null),
                    style: { background: C.border, border: "none", color: C.textM, fontSize: 11, cursor: "pointer", padding: "5px 12px", borderRadius: 7, fontFamily: "inherit" } }, "Отмена")
                )
              : React.createElement("button", {
      ...
      return React.createElement("div", null,
        ...
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } },
          [
            ...
            { id: "pelvic", l: "🌸 Таз. дно", sub: pelvicOpen ? "30 мин в день" : "С 1 июня" },
  ...
  function TodayTab({ todayDow, DR, setTab, workoutDays, cycleAnchor, packAnchor, periodOverrides, badDay, userName }) {
    ...
    let takenCount = 0;
    try {
      const stored = localStorage.getItem("pillsListV6");
      const taken = JSON.parse(localStorage.getItem("pillsTaken_" + todayKey) || "{}");
      const pills = stored ? JSON.parse(stored) : [];
      const dow = today.getDay() === 0 ? 6 : today.getDay() - 1;
      pills.forEach(p => {
        if (p.startDate && today < mkd(p.startDate)) return;
        if (p.weekdays && !p.weekdays.includes(dow)) return;
        if (p.ironRule && !isIronDay(today)) return;
        if (p.yarinaPack && packAnchor && !isYarinaActiveToday(today, packAnchor)) return;
        activeCount++;
    ...
    // До месячных
    const cycleDayNow = getCycleDay(today, cycleAnchor);
    const daysToPeriod = cycleDayNow >= 1 ? CYCLE_LEN - cycleDayNow + 1 : 0;
    ...
    return React.createElement("div", null,
      ...
      // Трекер воды (компактный)
      React.createElement(WaterTracker, null),

      // Трекер сна (один, без дубля)
      React.createElement(SleepTracker, null),

      // Трекер шагов (опциональный, появляется по тапу)
    ...
    if (hour >= 19) {
      try {
        ...
        const active = pills.filter(p => {
          if (p.startDate && t < mkd(p.startDate)) return false;
          if (p.weekdays && !p.weekdays.includes(dw)) return false;
          if (p.ironRule && !isIronDay(t)) return false;
          if (p.yarinaPack && !isYarinaActiveToday(t, packAnchor)) return false;
          return true;
        });

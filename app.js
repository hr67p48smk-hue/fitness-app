(() => {
  ...
  // ===========================================================================
  const KEY_DATES = {
    planStart:     new Date("2026-05-25"), // начало Недели 1 (пн)
    planEnd:       new Date("2026-07-26"), // конец Недели 9 (вс)
    // Подключения препаратов (даты согласованы с фарма-консультацией)
    forlaxStart:   new Date("2026-05-25"), // Форлакс — с 1-го дня плана
    vitDStart:     new Date("2026-05-25"), // Витамин D — с 1-го дня
    zincStart:     new Date("2026-05-30"), // Цинк — суббота недели 1
    niacinStart:   new Date("2026-06-01"), // Ниацинамид — пн недели 2
    pelvicStart:   new Date("2026-06-01"), // Курс тазового дна — пн недели 2
    perfectilStart:new Date("2026-06-04"), // Перфектил — чт недели 2
    ironStart:     new Date("2026-06-08"), // Железо — пн недели 3, через день
    ironDailyFrom: new Date("2026-06-22"), // Железо ежедневно — пн недели 5
    gymStart:      new Date("2026-06-10"), // Зал (ср + пт) — ср недели 3
    runStart:      new Date("2026-06-15"), // Бег / ходьба — пн недели 4
    pelvicEnd:     new Date("2026-08-03"), // Курс тазового дна — конец (пн, ~2 мес)
    dogLeaveDate:  new Date("2026-05-27"), // Собака уезжает к родителям (ср)
    dayCStart:     new Date("2026-07-06"), // День C — пн недели 7
    block7Start:   new Date("2026-07-06"), // Неделя 7 — анализы
    analysisRemindFrom: new Date("2026-06-29"), // напоминание сдать анализы за неделю
  };
  ...
  const PERIOD_AFTER_LAST_PILL = 4; // дней между последней таблеткой и началом месячных

  // Якорь цикла по умолчанию: первый день СЛЕДУЮЩИХ месячных = 8 июня 2026 (пн).
  // Это вычислено: последняя таблетка пачки 4 июня + 4 дня = 8 июня.
  function defaultCycleAnchor() {
    return "2026-06-08";
  }

  // Якорь пачки Ярины по умолчанию: первая таблетка ТЕКУЩЕЙ пачки.
  ...
  // Если сегодня ДО якоря — возвращаем "0" (цикл ещё не начался от этого якоря).
  function getCycleDay(date, anchorStr) {
    ...
    if (diff < 0) {
      // До якоря — считаем в предыдущем цикле (модуло 28)
  ...
  function getPackDay(date, packAnchorStr) {
    const anchor = new Date(packAnchorStr);
    anchor.setHours(0, 0, 0, 0);
    const d = new Date(date);
    d.setHours(0, 0, 0, 0);
    const diff = Math.floor((d - anchor) / 86400000);
  ...
  // Прогнозируемая дата начала N-го цикла = день 1 N-го цикла.
  // С учётом якоря (якорь = первый день месячных) — это тривиально.
  function getPredictedPeriodStart(n, anchorStr) {
    return getCycleStart(n, anchorStr);
  }
  ...
  // Возвращает день месячных (1..N) если сегодня попадает на месячные, иначе 0.
  ...
  function IronWindow() {
    ...
    const banStart = 9 * 60 + 30;  // 09:30
    const ironTime = 11 * 60 + 30; // 11:30
    const banEnd = 13 * 60 + 30;   // 13:30
    ...
    if (curMins < banStart) {
      ...
      hint = `Запретное окно начнётся в 09:30 (через ${leftH > 0 ? leftH + 'ч ' : ''}${leftM} мин). Успей последнюю чашку!`;
  ...
  function ProteinTracker() {
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 5, marginBottom: 6 } },
        [
          { v: 15, l: "+15", sub: "перекус" },
          { v: 20, l: "+20", sub: "лёгкий" },
          { v: 25, l: "+25", sub: "протеин" },
          { v: 30, l: "+30", sub: "полная" },
  ...
  function DayScheduleCard({ cycleAnchor, periodOverrides, workoutDays, packAnchor, onOpen }) {
    ...
    items.push({ time: "09:00", label: "Дуксет 60 мг", icon: "💊", kind: "pill" });
    items.push({ time: "09:30", label: "Завтрак", icon: "🍳", kind: "meal" });

    if (today >= KEY_DATES.forlaxStart) {
      items.push({ time: "10:30", label: "Форлакс", icon: "💊", kind: "pill" });
    }

    if (isIronDay(today)) {
      items.push({ time: "11:30", label: "Железо + витамин C (с водой!)", icon: "💊", kind: "pill_iron" });
    }

    // Обед
    const lunchPills = [];
    lunchPills.push("Омега 2 капс");
    lunchPills.push("Биофимейл");
    if (today >= KEY_DATES.vitDStart && (dow === 0 || dow === 2 || dow === 5)) lunchPills.push("Витамин D");
    if (today >= KEY_DATES.niacinStart) lunchPills.push("Ниацинамид");
    if (today >= KEY_DATES.perfectilStart) lunchPills.push("Перфектил");
    items.push({ time: "14:00", label: `Обед + ${lunchPills.join(", ")}`, icon: "🥗", kind: "meal" });
    ...
    if (today >= KEY_DATES.zincStart) {
      const zincTime = isTrainingDay ? "19:45" : "20:00";
      items.push({ time: zincTime, label: "Цинк", icon: "💊", kind: "pill" });
    }
    ...
    if (packAnchor && isYarinaActiveToday(today, packAnchor)) {
      items.push({ time: "21:30", label: "Ярина", icon: "💊", kind: "pill" });
    }
    items.push({ time: "22:00", label: "Мелатонин 2 мг", icon: "💊", kind: "pill" });
  ...
  function FullScheduleModal({ onClose, workoutDays, cycleAnchor, packAnchor, periodOverrides }) {
    ...
    items.push({ time: "09:00", label: "Дуксет 60 мг 💊", note: "Натощак, не привязано к еде" });
    items.push({ time: "09:30", label: "Завтрак 🍳", note: "Не позже 10:30 — белок 20-25 г" });
    if (today >= KEY_DATES.forlaxStart) {
      items.push({ time: "10:30", label: "Форлакс 💊", note: "Развести в стакане воды, выпить" });
    }
    if (isIronDay(today)) {
      items.push({ time: "11:30", label: "Железо + витамин C 💊", note: "ЗАПРЕТ кофе/чая/молочного 09:30—13:30!" });
    }
    const lunchPills = ["Омега 2 капс", "Биофимейл"];
    if (today >= KEY_DATES.vitDStart && (dow === 0 || dow === 2 || dow === 5)) lunchPills.push("Витамин D");
    if (today >= KEY_DATES.niacinStart) lunchPills.push("Ниацинамид");
    if (today >= KEY_DATES.perfectilStart) lunchPills.push("Перфектил");
    items.push({ time: "14:00", label: `Обед + ${lunchPills.join(", ")} 💊`, note: "Полноценная еда, белок 30-35 г" });
    ...
    if (today >= KEY_DATES.zincStart) {
      const zincTime = isTrainingDay ? "19:45" : "20:00";
      items.push({ time: zincTime, label: "Цинк 💊", note: "С ужином или сразу после" });
    }
    ...
    if (packAnchor && isYarinaActiveToday(today, packAnchor)) {
      items.push({ time: "21:30", label: "Ярина 💊", note: "Строго в одно время" });
    }
    items.push({ time: "22:00", label: "Мелатонин 2 мг 💊", note: "За час до сна, без экранов после" });
    items.push({ time: "22:00", label: "Готовлюсь ко сну 🌙", note: "Душ, свет приглушённый" });
  ...
  function getTodayActivity({ date, cycleAnchor, periodOverrides, workoutDays, badDay }) {
    ...
    if (today < KEY_DATES.planStart) {
      ...
      return {
        ...
        hint: "План восстановления начинается 25 мая. Сейчас — отдых, привычный режим.",
        icon: "🌱"
      };
    }
    ...
    // С 10 июня — силовые в ср + пт (workoutDays = [2, 4])
    ...
    if (isWorkoutDay) {
      ...
      if (widx === 0) return {
        kind: "gym_a",
        label: "Зал — День A",
        hint: "10 мин разминка + 40 мин силовая (ягодицы+кор) + 30 мин таз. дна. Минимальные веса.",
        icon: "🍑"
      };
      if (widx === 1) return {
        kind: "gym_b",
        label: "Зал — День B",
        hint: "10 мин разминка + 40 мин силовая (спина+кор) + 30 мин таз. дна. Без жима лёжа.",
        icon: "🦅"
      };
      if (widx === 2 && dayCAvailable) return {
        kind: "gym_c",
        label: "Зал — День C",
        hint: "10 мин разминка + 40 мин силовая (ягодицы+ноги) + 30 мин таз. дна. Высокая интенсивность.",
        icon: "🔥"
      };
    }
  ...
  function PillsModule({ compact, cycleAnchor, packAnchor }) {
    ...
    // Все startDate привязаны к понедельникам (кроме цинка — сб 30 мая).
    // Все даты — план «как обсуждали с фарма-консультацией».
    const DEFAULT_PILLS = [
      // 09:00 — Дуксет натощак (просто по времени, не с едой)
      { id: "duxet",      name: "Дуксет 60 мг",         time: "09:00", color: MORNING, bg: MORNING_BG,
        note: "Антидепрессант (дулоксетин). Просто в 9:00, не привязано к еде. Принимаешь больше полугода — режим не менять, не пропускать. При резкой отмене — синдром отмены." },
      // 10:30 — Форлакс (через 1.5 ч после Дуксета)
      { id: "forlax",     name: "Форлакс 1 пакет",      time: "10:30", color: MORNING, bg: MORNING_BG,
        note: "Осмотическое слабительное. Развести в стакане воды, выпить. Работает мягко через сутки. Старт с 25 мая.",
        startDate: "2026-05-25" },
      // 11:30 — Железо (через день в начале, потом ежедневно)
      { id: "iron",       name: "Gentle Iron 25 мг + Витамин C", time: "11:30", color: PRELUNCH, bg: PRELUNCH_BG,
        note: "С 8 июня. ВТ/ЧТ/СБ первые 2 недели, с 22 июня — ежедневно. Запивать водой, не кофе/чаем/молочным (2 ч ДО и ПОСЛЕ — нельзя!). Витамин C усиливает усвоение. Чёрный стул — норма.",
        startDate: "2026-06-08",
        ironRule: true /* флаг — особая логика "только в дни железа" */ },
      // 14:00 — Обеденный блок (все вместе)
      { id: "omega",      name: "Омега-3 (2 капсулы)",  time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "С обедом, 2 капсулы. Помогает усвоению железа и противовоспалительный эффект." },
      { id: "biofemale",  name: "Биофимейл",            time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "Пробиотик для женщин. С обедом. Защищает микрофлору при приёме железа." },
      { id: "vitd",       name: "Витамин D 4000 МЕ",    time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "Только ПН / СР / СБ (3 раза в неделю). С жирной едой — лучше усваивается.",
        startDate: "2026-05-25",
        weekdays: [0, 2, 5] /* пн, ср, сб (0=пн в моей нумерации) */ },
      { id: "niac",       name: "Ниацинамид 500 мг",    time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "Для волос. С обедом — лучше переносится. У ниацинамида нет flushing-эффекта (не путать с никотиновой кислотой).",
        startDate: "2026-06-01" },
      { id: "perfectil",  name: "Перфектил",            time: "14:00", color: LUNCH, bg: LUNCH_BG,
        note: "Только после полноценного обеда! Натощак — тошнота. Содержит железо, цинк, B-группу, витамины A/E.",
        startDate: "2026-06-04" },
      // 20:00 — Цинк
      { id: "zinc",       name: "Цинк ~15 мг",          time: "20:00", color: EVENING, bg: EVENING_BG,
        note: "С ужином или сразу после. Между цинком и железом минимум 2 часа разрыв (у нас 8.5 ч — идеально).",
        startDate: "2026-05-30" },
      // 21:30 — Ярина (по графику пачки)
      { id: "yarina",     name: "Ярина",                time: "21:30", color: NIGHT, bg: NIGHT_BG,
        note: "Строго в 21:30. 21 таблетка + 7 дней перерыв. Приложение само скроет в дни перерыва.",
        yarinaPack: true /* флаг — особая логика "только в дни пачки" */ },
      // 22:00 — Мелатонин
      { id: "mel",        name: "Мелатонин 2 мг",       time: "22:00", color: NIGHT, bg: NIGHT_BG,
        note: "За час до сна. После приёма не лезть в экраны." },
    ];

    const todayKey = new Date().toLocaleDateString("ru-RU");
    // V4 — потому что я снова переделал структуру (новые времена, ironRule, yarinaPack)
    const [pills, setPills] = useLS("pillsListV4", DEFAULT_PILLS);
  ...
  function CycleSubTab({ cycleAnchor: extAnchor, packAnchor: extPack }) {
    const [cycleAnchor, setCycleAnchor] = useLS("cycleAnchorV1", defaultCycleAnchor());
    const [packAnchor, setPackAnchor] = useLS("packAnchorV1", defaultPackAnchor());
    ...
    today.setHours(0, 0, 0, 0);
    const todayCycleDay = getCycleDay(today, cycleAnchor);
    const todayPackDay = getPackDay(today, packAnchor);
    const yarinaActive = isYarinaActiveToday(today, packAnchor);

    // Какой сейчас цикл / какая пачка
    const cycleAnchorDate = new Date(cycleAnchor); cycleAnchorDate.setHours(0, 0, 0, 0);
    const packAnchorDate = new Date(packAnchor); packAnchorDate.setHours(0, 0, 0, 0);
    const currentCycleNum = Math.floor((today - cycleAnchorDate) / 86400000 / CYCLE_LEN) + 1;
    const currentPackNum = Math.floor((today - packAnchorDate) / 86400000 / CYCLE_LEN) + 1;

    // Прогноз будущих циклов (месячные)
    const cycles = [currentCycleNum, currentCycleNum + 1, currentCycleNum + 2, currentCycleNum + 3]
      .filter(n => n >= 1)
      .map(n => {
        const start = getCycleStart(n, cycleAnchor);
        const ov = periodOverrides[n];
        const periodStart = ov ? new Date(ov) : start;
        periodStart.setHours(0, 0, 0, 0);
        const periodEnd = new Date(periodStart);
        periodEnd.setDate(periodEnd.getDate() + PERIOD_LENGTH - 1);
        return { n, periodStart, periodEnd, isOverride: !!ov, isCurrent: n === currentCycleNum };
      });
      ...
      return React.createElement("div", { style: { background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 12, padding: "13px", marginBottom: 12 } },
        ...
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2 } },
          cells.map((d, i) => {
            ...
            const packDay = getPackDay(d, packAnchor);
            const isYarinaDay = packDay >= 1 && packDay <= ACTIVE_PILLS;
            const isYarinaBreak = packDay > ACTIVE_PILLS;
            // Цвет фона ячейки
            let bg = "transparent", fg = C.text;
            if (periodDay >= 1 && periodDay <= PERIOD_LENGTH) { bg = C.pinkSoft; fg = C.pink; }
            else if (isYarinaBreak) { bg = "#FBF3F6"; fg = C.pink; }
            else if (isYarinaDay) { bg = "#F2F0FE"; fg = "#534AB7"; }
            const border = isToday ? `1.5px solid ${C.olive}` : "0.5px solid transparent";
            return React.createElement("div", { key: i,
              style: { aspectRatio: "1", background: bg, border, borderRadius: 6,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 11, color: fg, fontWeight: isToday ? 700 : 400 }
            }, d.getDate());
          })
        ),
        // Легенда
        React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "8px 14px", marginTop: 10, fontSize: 10, color: C.textM } },
          [
            { c: "#F2F0FE", l: "Таблетка пачки" },
            { c: "#FBF3F6", l: "Перерыв Ярины" },
            { c: C.pinkSoft, l: "Месячные" },
    ...
    return React.createElement("div", null,
      ...
      React.createElement("div", { style: { background: C.card, borderRadius: 12, padding: "13px 14px", marginBottom: 10, border: `0.5px solid ${C.border}` } },
        ...
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 } },
          ...
          React.createElement("div", { style: { background: C.pinkSoft, borderRadius: 9, padding: "10px 11px", border: `0.5px solid ${C.pink}33` } },
            React.createElement("div", { style: { fontSize: 10, color: C.pink, fontWeight: 600, letterSpacing: 0.3 } }, "ДЕНЬ ЦИКЛА"),
            React.createElement("div", { style: { fontSize: 22, fontWeight: 700, color: C.pink, marginTop: 2, lineHeight: 1 } }, todayCycleDay),
            React.createElement("div", { style: { fontSize: 10, color: C.textM, marginTop: 4 } }, "из 28")
          ),
          // Пачка
      ...
      React.createElement("div", { style: { background: C.bgWarm, borderRadius: 10, padding: "11px 13px", marginTop: 14 } },
        React.createElement("div", { style: { fontSize: 11, color: C.textM, marginBottom: 6 } }, "Первый день текущих/последних месячных"),
        React.createElement("input", { type: "date", value: cycleAnchor, onChange: e => setCycleAnchor(e.target.value),
        ...
        React.createElement("div", { style: { fontSize: 11, color: C.textM, marginTop: 12, marginBottom: 6 } }, "Первая таблетка текущей пачки Ярины"),
    ...
    return React.createElement("div", null,
      ...
      React.createElement("div", { style: { background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 12, padding: "13px 14px", marginBottom: 12 } },
        ...
        React.createElement("div", { style: { fontSize: 11, color: C.textL, marginTop: 8, lineHeight: 1.5 } },
          "💡 Серединная сдача — за неделю до Блока 7 (29 июня). Финальная — после плана (26 июля)."
        )
      ),
  ...
  // ===========================================================================
  function SportTab({ workoutDays, doneCount, weekLog, markDay, DR, todayDow, selDay, setSelDay, dayCAvailable, setWorkoutDays }) {
    ...
    const renderTodayCard = () => {
      ...
      if (isTrain) {
        const widx = workoutDays.indexOf(dow);
        const dayName = widx === 0 ? "День A (Ягодицы + Кор)" : widx === 1 ? "День B (Спина + Кор)" : "День C (Ягодицы + Ноги)";
  ...
  function SettingsTab({ badDay, setBadDay, setOnboardingDone }) {
    ...
    return React.createElement("div", null,
      ...
      activeSec === "profile" && Card({ children: React.createElement(React.Fragment, null,
        ...
        }), hint: "На день старта плана (25 мая 2026). Норма 30-100." }),
  ...
  function TodayTab({ todayDow, DR, setTab, workoutDays, cycleAnchor, packAnchor, periodOverrides, badDay, userName }) {
    ...
    let takenCount = 0;
    try {
      const stored = localStorage.getItem("pillsListV4");
    ...
    const daysToStart = beforePlan ? Math.ceil((KEY_DATES.planStart - today) / 86400000) : 0;
    ...
    return React.createElement("div", null,
      ...
      beforePlan && React.createElement("div", {
        ...
        React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: C.text, marginTop: 3, position: "relative" } },
          daysToStart === 1 ? "Завтра!" : `${daysToStart} ${daysToStart < 5 ? 'дня' : 'дней'}`
        ),
        React.createElement("div", { style: { fontSize: 11, color: C.textM, marginTop: 4, lineHeight: 1.5, position: "relative" } },
          "Старт 25 мая (понедельник). Сегодня — отдых и привычный режим. К завтра подготовь: купи Форлакс и витамин D, поставь будильник на 8:00."
        )
      ),
      ...
      // Режим дня (компактный таймлайн) — только если план начался
      !beforePlan && React.createElement(DayScheduleCard, {
  ...
  function OnboardingScreen({ onDone }) {
    ...
    const [cycleAnchorVal, setCycleAnchorVal] = useState(defaultCycleAnchor());
    const [ferritin, setFerritin] = useState("");
    const [step, setStep] = useState(0);

    // Когда юзер вводит дату первой таблетки пачки — автоматически рассчитываем
    // предполагаемую дату месячных (последняя таблетка + 4 дня)
    const onPackChange = (v) => {
      setPackAnchorVal(v);
      // Прогноз месячных = (v + 21 + 4 дн) → используем как cycleAnchor
      try {
        const d = new Date(v);
        d.setDate(d.getDate() + ACTIVE_PILLS + PERIOD_AFTER_LAST_PILL - 1);
        setCycleAnchorVal(d.toISOString().slice(0, 10));
      } catch {}
    };
    ...
    const save = () => {
      try {
        ...
        localStorage.setItem("packAnchorV1", JSON.stringify(packAnchorVal));
        localStorage.setItem("cycleAnchorV1", JSON.stringify(cycleAnchorVal));
    ...
    if (step === 1) {
      const packDate = new Date(packAnchorVal);
      const lastPill = new Date(packDate); lastPill.setDate(lastPill.getDate() + ACTIVE_PILLS - 1);
      const predictedPeriod = new Date(lastPill); predictedPeriod.setDate(predictedPeriod.getDate() + PERIOD_AFTER_LAST_PILL);
      const fmt = d => d.toLocaleDateString("ru-RU", { day: "numeric", month: "long" });

      return wrap(React.createElement("div", null,
        React.createElement("div", { style: { fontSize: 20, fontWeight: 600, color: C.text, marginBottom: 8 } }, "Ярина"),
        React.createElement("div", { style: { fontSize: 13, color: C.textM, lineHeight: 1.6, marginBottom: 18 } },
          "Когда была первая таблетка ", React.createElement("b", null, "текущей пачки"), "? Месячные обычно начинаются через 4 дня после последней таблетки — я сама посчитаю и предложу."
        ),
        React.createElement("input", { type: "date", value: packAnchorVal, onChange: e => onPackChange(e.target.value),
          style: { ...dateInputStyle, marginBottom: 10 }
        }),
        React.createElement("div", { style: { fontSize: 11, color: C.textL, lineHeight: 1.6, marginBottom: 18, padding: "10px 12px", background: C.bgWarm, borderRadius: 8 } },
          "Последняя (21-я) таблетка: ", React.createElement("b", { style: { color: C.olive } }, fmt(lastPill)), React.createElement("br"),
          "Прогноз следующих месячных: ", React.createElement("b", { style: { color: C.pink } }, fmt(predictedPeriod))
        ),
        React.createElement("div", { style: { display: "flex", gap: 8 } },
  ...
  const PLAN_BLOCKS = [
    {
      ...
      sections: [
        { h: "Что подключается на этой неделе", items: [
          "ПН 25 мая — Форлакс 1 пакет в 10:30",
          "ПН 25 мая — Витамин D 4000 МЕ (пн/ср/сб в обед)",
          "СБ 30 мая — Цинк ~15 мг в 20:00"
        ], ok: true },
        { h: "Уже принимаешь стабильно", items: [
          "Дуксет 60 мг в 09:00",
          "Ярина в 21:30",
          "Омега-3 (2 капс), Биофимейл — в обед",
          "Мелатонин 2 мг в 22:00"
        ]},
        { h: "Активность", items: [
        ...
        { h: "Купить до 1 июня", items: [
          "Перфектил",
          "Ниацинамид 500 мг",
          "Gentle Iron 25 мг + витамин C",
          "Записаться на курс таз. дна онлайн"
        ]},
    ...
    },
    {
      ...
      goal: "Добавить Перфектил, Ниацинамид, курс таз. дна. Главное — ЖКТ должен справиться.",
      sections: [
        { h: "Что подключается", items: [
          "ПН 1 июня — Ниацинамид 500 мг (в обед 14:00)",
          "ПН 1 июня — Курс таз. дна (6 раз в неделю, 30 мин, отдых в вс)",
          "ЧТ 4 июня — Перфектил (только с обедом! Натощак — тошнота)"
        ], ok: true },
        { h: "Важно про Перфектил", items: [
          "Только после полноценного обеда",
          "В составе уже есть железо, цинк, B-группа — поэтому нужен между ним и отдельным железом разрыв 2 часа",
          "Если тошнит — попробовать с большим количеством воды"
        ], warn: true },
        { h: "Курс таз. дна", items: [
          "ПН-СБ по 30 мин в 18:00",
          "Воскресенье — отдых",
        ...
        { h: "Цели на конец недели", items: [
          ...
          "Курс таз. дна 6/7 дней выполнен",
          "Месячные начались ~8 июня — записать факт"
        ], ok: true },
      ]
    },
    {
      ...
      title: "8 — 14 июня",
      subtitle: "Железо и зал",
      goal: "Подключить железо и начать силовые. Это самая нагруженная неделя — слушать тело.",
      sections: [
        { h: "Что подключается", items: [
          "ПН 8 июня — Gentle Iron 25 мг + витамин C в 11:30 (вт/чт/сб через день)",
          "СР 10 июня — Зал 18:00 (разминка 10 мин + силовая 40 мин)",
          "СР 10 июня — Курс таз. дна 30 мин в зале после силовой"
        ], ok: true },
        { h: "Железо — главные правила", items: [
          "В 11:30 натощак, между завтраком и обедом",
          "ЗАПРЕТ кофе/чая/молочного 09:30—13:30 в дни приёма",
          "С 8 июня по 21 июня — только ВТ/ЧТ/СБ (через день)",
          "С 22 июня — ежедневно"
        ], warn: true },
        { h: "Месячные на этой неделе", items: [
          "8—12 июня — месячные (день 1 цикла = 8 июня)",
          "Первые 3 дня (8, 9, 10 июня) — лёгкая интенсивность",
          "В ПН 8 июня зал НЕ начинаем — слишком интенсивно. Первая силовая = СР 10 июня"
        ]},
        { h: "Зал — старт", items: [
          "Программа: 10 мин разминка + 40 мин силовая + 30 мин таз. дна",
    ...
    },
    {
      ...
      sections: [
        { h: "Что подключается", items: [
          "ПН 15 июня — Бег/ходьба 1 раз в неделю (по самочувствию, в субботу или вне дней зала)",
    ...
    },
    {
      ...
      goal: "Железо переходит на ежедневный приём. Это значит запретное окно кофе/чая/молочного каждый день.",
      sections: [
        { h: "Изменение режима", items: [
          "ПН 22 июня — Железо теперь ЕЖЕДНЕВНО в 11:30",
          "Запретное окно 09:30—13:30 каждый день (нельзя кофе/чай/молочное)",
          "Утренний зелёный чай — только до 09:30!"
        ], warn: true },
  ...
  function PlanTab() {
    ...
    const [packAnchor] = useLS("packAnchorV1", defaultPackAnchor());
    ...
    // Текущая неделя плана — для шапки
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { style: { padding: "14px 14px 0" } },
        ...
        tab === "sport" && React.createElement(SportTab, { workoutDays, setWorkoutDays, doneCount, weekLog, markDay, DR, todayDow, selDay, setSelDay, dayCAvailable }),

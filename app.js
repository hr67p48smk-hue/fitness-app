  ...
  function StepsTracker() {
      ...
      return React.createElement("button", {
        ...
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
          React.createElement("div", null,
            ...
            React.createElement("div", { style: { fontSize: 11, color: C.textM, marginTop: 2 } }, "Нажми, чтобы записать")
          ),
          React.createElement("div", { style: { fontSize: 20, color: C.olive, fontWeight: 300 } }, "+")
        )
      );
    }
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
        ...
        React.createElement("div", { style: { fontSize: 18, fontWeight: 700, color: C.olive } }, todaySteps.toLocaleString("ru-RU"))
      )
    );
  }
  ...
  function SportPlanner() {
    ...
    return React.createElement("div", null,
      ...
      dayLabels.map((dl, di) => {
        ...
        const hasPlan = acts.length > 0 || pelvic;
        // Сводка дня (свёрнуто)
        const summaryBits = acts.map(a => { const s = SPORT_ACTS.find(x => x.id === a.id); return s ? s.icon : ""; });
        if (pelvic) summaryBits.push("🌸");

        return React.createElement("div", { key: di, style: { background: isOpen ? C.bgWarm : C.card, border: `0.5px solid ${isToday ? C.olive : C.border}`, borderRadius: 12, marginBottom: 8, overflow: "hidden" } },
          // Заголовок дня — тап раскрывает
          React.createElement("button", { onClick: () => setOpenDay(isOpen ? -1 : di),
            style: { width: "100%", textAlign: "left", background: "transparent", border: "none", padding: "12px 14px", cursor: "pointer", fontFamily: "inherit", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 } },
            React.createElement("div", { style: { display: "flex", alignItems: "baseline", gap: 8, minWidth: 0 } },
              React.createElement("span", { style: { fontSize: 13, fontWeight: 700, color: isToday ? C.oliveDeep : C.text } }, isOpen ? dl : dayShort[di], " ", dayDate.getDate()),
              isToday && React.createElement("span", { style: { fontSize: 10, fontWeight: 700, color: "#fff", background: C.olive, borderRadius: 999, padding: "2px 7px" } }, "сегодня")
            ),
            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 7, flexShrink: 0 } },
              !isOpen && (hasPlan
                ? React.createElement("span", { style: { fontSize: 13 } }, summaryBits.join(" "))
                : React.createElement("span", { style: { fontSize: 10.5, color: C.textL } }, "отдых")),
          ...
          isOpen && React.createElement("div", { style: { padding: "0 14px 14px" } },
            ...
            React.createElement("div", { style: { display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 4 } },
              SPORT_ACTS.map(a => {
            ...
            // Для каждой выбранной активности — время
            acts.map(a => {
      ...
      React.createElement("div", { style: { fontSize: 10.5, color: C.textL, lineHeight: 1.5, marginTop: 4, padding: "10px 12px", background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 9 } },
        "Тапни день, чтобы раскрыть и выбрать тренировки, время суток и точное время. Тазовое дно — каждый день утром или вечером (курс с 15 июня). Запланированное появится в «Сегодня».")
    );
  }
  ...
  function TodayTab({ todayDow, DR, setTab, workoutDays, cycleAnchor, packAnchor, periodOverrides, badDay, userName }) {
    ...
    return React.createElement("div", null,
      ...
      !diaryDone && !showDiary && React.createElement("button", {
        ...
        style: { width: "100%", textAlign: "left", background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", marginBottom: 10,
          display: "flex", alignItems: "center", gap: 12, cursor: "pointer", fontFamily: "inherit" }
      },
        React.createElement("div", { style: { width: 32, height: 32, borderRadius: "50%", background: C.oliveSoft, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 } }, "🌿"),
        React.createElement("div", { style: { flex: 1 } },
          React.createElement("div", { style: { fontSize: 13, fontWeight: 600, color: C.text } }, "Как ты сегодня?"),
          React.createElement("div", { style: { fontSize: 11, color: C.textM, marginTop: 1 } }, "минутка внимания к себе")
        ),

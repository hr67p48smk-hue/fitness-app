      ...
      React.createElement("div", { style: { borderRadius: 12, overflow: "hidden", border: `0.5px solid ${C.border}`, marginBottom: 12 } },
        dayLabels.map((dl, di) => {
          ...
          const renderPill = (it, k) => {
            ...
            return React.createElement("span", { key: k, style: { fontSize: 11.5, fontWeight: 600, color: it.kind === "pelvic" ? C.pink : C.text, background: it.color + (it.kind === "pelvic" ? "1e" : "22"), borderRadius: 999, padding: "3px 9px", whiteSpace: "nowrap" } }, it.icon, " ", it.label, when ? (" · " + when) : "");
          };

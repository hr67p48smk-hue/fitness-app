(() => {
  ...
  function FoxNavIcon({ kind, size = 28, active }) {
    const EMOJI = { nutrition: "🥗" };
    if (EMOJI[kind]) {
      return React.createElement("div", {
        style: { fontSize: size - 6, lineHeight: 1, opacity: active ? 1 : 0.42,
          filter: active ? "none" : "grayscale(40%)", transition: "opacity .15s" }
      }, EMOJI[kind]);
    }
  ...
  function MoodDiary({ onClose }) {
    ...
    return React.createElement("div", { style: { background: C.card, borderRadius: 14, padding: "16px", border: `0.5px solid ${C.border}` } },
      ...
      Section({ title: "Сон", children: React.createElement("div", { style: { fontSize: 12, color: C.textM, lineHeight: 1.5, padding: "10px 12px", background: C.bgWarm, borderRadius: 8 } },
        sleepH > 0
          ? React.createElement(React.Fragment, null, "Сегодня отмечено: ", React.createElement("b", { style: { color: C.oliveDeep } }, sleepH, " ч"), ". Изменить — в трекере сна на вкладке «Сегодня».")
          : "Часы сна отмечаются в трекере сна на вкладке «Сегодня» — чтобы не вводить дважды."
      )}),

(() => {
  ...
  function isIronDay(date) {
    ...
    // Первые 2 недели — через день, отсчёт от старта (ср/пт/вс): чётное число дней от ironStart
  ...
  const DEFAULT_PILLS = [
    { id: "iron", name: "Gentle Iron 25 мг + Витамин C", time: "08:15", color: PC.MORNING, bg: PC.MORNING_BG,
      note: "Натощак, за 30–45 мин до завтрака. Через день (СР/ПТ/ВС) с 3 июня, ежедневно с 17 июня. Запивать водой или соком — НЕ кофе/чаем/молочным (1–2 ч до и после нельзя). Витамин C усиливает усвоение. Чёрный стул — норма. Gentle Iron (бисглицинат) обычно мягче для ЖКТ.",
      startDate: "2026-06-03", ironRule: true },
    ...
    { id: "perfectil", name: "Перфектил", time: "14:00", color: PC.LUNCH, bg: PC.LUNCH_BG,
      note: "Только после полноценного обеда! Натощак — тошнота. Содержит железо, цинк, B-группу, витамины A/E. Доказательная база мультивитаминов «для волос» слабая — это поддержка, а не лечение; главное в твоём плане работает не за счёт него. ⚠️ Содержит витамин А — не дублируй с A+E (см. заметку A+E). С 6 июня.",
    ...
    { id: "forlax", name: "Форлакс 1 пакет", time: "15:00", color: PC.LUNCH, bg: PC.LUNCH_BG,
  ...
  const INTAKE_RULES = {
    ...
    forlax: { can: ["Развести в стакане воды", "Пить больше воды днём"], cant: ["Вплотную к другим таблеткам"], gapBefore: 1, gapAfter: 1, gapReason: "другие таблетки" },
    ...
    biofemale: { can: ["На ночь, прохладной/тёплой водой"], cant: ["Не запивать горячим"] },
  ...
  const INTAKE_CONFLICTS = [
    ...
    ["iron", "forlax", "Форлакс отдельно от железа (1–2 ч)"],
  ];
  ...
  function PillsModule({ compact, cycleAnchor, packAnchor }) {
    ...
    const toggle = (id) => {
      ...
      const at = { ...(taken.__at || {}) };
      if (!taken[id]) at[id] = nowM; else delete at[id];
      next.__at = at;
    ...
    const markAllTaken = () => {
      ...
      const next = { ...taken };
      const at = { ...(taken.__at || {}) };
      activePills.forEach(p => { next[p.id] = true; if (at[p.id] == null) at[p.id] = nowM; });
      next.__at = at;
      setTaken(next);
    };
    ...
    const _up = activePills.filter(p => !taken[p.id]).map(p => ({ p, t: _pt(p.time) })).filter(x => x.t >= _cur - 20).sort((a, b) => a.t - b.t)[0];
    const nextLine = _up
      ? "Ближайшее: " + _up.p.name + " — " + (Math.abs(_up.t - _cur) <= 10 ? "сейчас" : (_up.t > _cur ? "через " + _hu(_up.t - _cur) : "пора"))
      : (activePills.length > 0 && doneCount === activePills.length ? "Всё на сегодня принято ✓" : null);
    // Таймер железа
    // Таймер железа — от фактического времени приёма (taken.__at.iron), редактируемого
    let ironTimerText = null;
    if (taken.iron && taken.__at && taken.__at.iron != null) {
      const since = _cur - taken.__at.iron; // минут прошло от фактического приёма
    ...
    // Конфликты (близко по времени, не оба приняты)
    const conflictNotes = [];
    ...
    INTAKE_CONFLICTS.forEach(([a, b, why]) => {
      ...
      if (!ia || !ib) return;
      if (taken[a] && taken[b]) return; // оба приняты — напоминать поздно
      if (Math.abs(effT(ia) - effT(ib)) < 120) conflictNotes.push(why);
    });
    // Вечерняя сводка
    const eveningLeft = _now.getHours() >= 19 ? activePills.filter(p => !taken[p.id]) : [];
    ...
    const renderRules = (p) => {
      ...
      if (!r) return React.createElement("div", null, p.note || "Нет заметок");
      ...
      return React.createElement(React.Fragment, null,
        ...
        // Фактическое время приёма — можно поправить (только для препаратов с окнами)
        isTaken && (r.gapBefore || r.gapAfter) && React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginTop: 7, paddingTop: 7, borderTop: `0.5px solid ${C.border}` } },
          ...
          fromActual ? React.createElement("span", { style: { fontSize: 10, color: C.oliveDeep } }, "✓ от факта") : React.createElement("span", { style: { fontSize: 10, color: C.textL } }, "(план)")
        ),
        ...
        React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 } },
          activePills.map(p => React.createElement("button", {
            key: p.id, onClick: () => {
              ...
              const at = { ...(taken.__at || {}) };
              if (!taken[p.id]) at[p.id] = nowM; else delete at[p.id];
              next.__at = at;
...
const RECIPES = [{"id": "b1", "ch": "breakfast", "name": "Жареные яйца с перцем и хлебом", "time": 7, "tags": ["до 20 мин"], "kbju": {"k": 160, "p": 10, "f": 10, "c": 8}, "ing": ["Яйца — 2 шт (120г)", "Болгарский перец — ½ шт (80г)", "Хлеб цельнозерновой — 2 куска (60г)", "Масло оливковое/сливочное — ½ ч.л. (3г)", "Соль, чёрный перец"], "steps": ["Перец нарезать кольцами или соломкой.", "Сковороду разогреть на среднем огне, добавить масло.", "Выложить перец, обжарить 2–3 мин до лёгкой мягкости.", "Сдвинуть перец, разбить яйца рядом.", "Жарить 2–3 мин — белок схватился, желток по желанию.", "Посолить, поперчить. Подавать с хлебом."], "masha": "1 яйцо + 1 кусок хлеба (~240 ккал, 14г белка). Или 2 яйца без хлеба (~260 ккал, 18г). В Фазу 1 удобнее без хлеба.", "tip": "Идеальная прожарка — края белка хрустят, желток жидкий. Капля воды + крышка на 30 сек — желток схватится сверху.", "why": ""}, {"id": "b2", "ch": "breakfast", "name": "Шакшука — яйца в томатном соусе", "time": 20, "tags": ["до 20 мин"], "kbju": {"k": 85, "p": 5, "f": 5, "c": 6}, "ing": ["Яйца — 4 шт (240г)", "Помидоры — 3 шт (450г) или консерв. 400г", "Болгарский перец — 1 шт (150г)", "Лук — 1 шт (100г)", "Чеснок — 2 зубчика", "Паприка — 1 ч.л.", "Тмин молотый — ½ ч.л.", "Оливковое масло — 1 ст.л.", "Соль, перец", "По желанию: Фета 40г, петрушка/кинза"], "steps": ["Лук нарезать кубиком, перец полосками, чеснок раздавить.", "Обжарить лук на масле 4–5 мин. Добавить перец, ещё 3 мин.", "Добавить чеснок, паприку, тмин — 1 мин, помешивая.", "Добавить помидоры. Тушить 8–10 мин до загустения. Посолить.", "Сделать 4 углубления, разбить в каждое яйцо.", "Накрыть крышкой, готовить 4–5 мин — белок схватился, желток жидкий.", "Посыпать Фетой и зеленью."], "masha": "Та же порция — 2 яйца + половина соуса. КБЖУ одинаковые.", "tip": "Тмин — обязательная специя. Консервированные помидоры дают более насыщенный вкус.", "why": ""}, {"id": "b3", "ch": "breakfast", "name": "Протеиновые блинчики без муки", "time": 15, "tags": ["до 20 мин", "белок"], "kbju": {"k": 135, "p": 11, "f": 6, "c": 9}, "ing": ["Яйца — 3 шт (180г)", "Творог 5% — 100г", "Банан спелый — ½ шт (60г)", "Корица — ½ ч.л.", "Масло сливочное — ½ ч.л."], "steps": ["Пробить яйца, творог, банан и корицу блендером до гладкого теста.", "Сковороду разогреть, смазать маслом.", "Налить порциями (~3 ст.л. = 1 блинчик).", "Жарить 2 мин до пузырей. Перевернуть, ещё 1,5 мин.", "Получается ~6 небольших блинчиков."], "masha": "4 блинчика (~310 ккал, 25г белка). Отличный вариант в тренировочный день.", "tip": "Банан должен быть спелым — даёт сладость и связывает тесто.", "why": ""}, {"id": "b4", "ch": "breakfast", "name": "Белковые вафли в вафельнице", "time": 15, "tags": ["до 20 мин", "белок"], "kbju": {"k": 145, "p": 10, "f": 5, "c": 14}, "ing": ["Яйца — 2 шт (120г)", "Творог 5% — 150г", "Овсяные хлопья — 3 ст.л. (30г)", "Банан — ½ шт (60г)", "Разрыхлитель — ½ ч.л.", "Ванилин — щепотка"], "steps": ["Пробить все ингредиенты блендером до однородности.", "Дать постоять 3 мин — хлопья набухнут.", "Вафельницу разогреть, слегка смазать маслом.", "Выпекать порциями 3–4 мин до золотистого цвета.", "Получается ~3 вафли."], "masha": "2 вафли (~345 ккал, 24г белка). Хорошо после тренировки.", "tip": "Тесто густое — не растекается. Хрустят сразу из вафельницы.", "why": ""}, {"id": "b5", "ch": "breakfast", "name": "Яичные кексы с овощами (заготовка)", "time": 25, "tags": ["заготовка", "белок"], "kbju": {"k": 115, "p": 9, "f": 7, "c": 3}, "ing": ["Яйца — 6 шт (360г)", "Болгарский перец — ½ шт (75г)", "Шпинат свежий — горсть (30г)", "Помидоры черри — 6 шт", "Фета — 40г", "Соль, перец", "Форма для маффинов — 6 ячеек"], "steps": ["Разогреть духовку до 180°С.", "Взбить яйца с солью и перцем.", "Мелко нарезать перец и шпинат, выложить в формочки.", "В каждую — половинка черри и щепотка Феты.", "Залить яйцом на ¾ высоты.", "Духовка 180°С 18–20 мин (аэрогриль 170°С 14–16 мин)."], "masha": "3 кекса к завтраку — быстро и питательно. Хранятся 4 дня.", "tip": "Готовить 6–12 штук в воскресенье — разогревать 1 мин в микроволновке.", "why": ""}, {"id": "b6", "ch": "breakfast", "name": "Яйцо пашот", "time": 5, "tags": ["до 20 мин", "белок"], "kbju": {"k": 155, "p": 13, "f": 11, "c": 1}, "ing": ["Яйца свежие — 2 шт", "Уксус 9% — 1 ст.л.", "Вода — 1л", "Соль — щепотка"], "steps": ["Воду довести до лёгкого кипения (~90°С).", "Добавить уксус и щепотку соли.", "Каждое яйцо разбить в чашку.", "Ложкой сделать воронку, влить яйцо в центр.", "Варить 3 мин (жидкий желток) или 4 мин (полутвёрдый).", "Вынуть шумовкой, промокнуть."], "masha": "185 ккал, 16г белка — отличный лёгкий белковый завтрак.", "tip": "Уксус помогает белку свернуться. Если яйцо растекается — оно несвежее.", "why": ""}, {"id": "b7", "ch": "breakfast", "name": "Авокадо-тост с яйцом", "time": 10, "tags": ["до 20 мин"], "kbju": {"k": 175, "p": 7, "f": 10, "c": 14}, "ing": ["Хлеб цельнозерновой — 2 куска (60г)", "Авокадо — ½ шт (80г)", "Яйцо варёное/пашот — 1 шт", "Помидор — 1 шт", "Лимонный сок — 1 ч.л.", "Оливковое масло — 1 ч.л.", "Соль, перец, чили"], "steps": ["Хлеб поджарить.", "Авокадо размять с лимонным соком, солью, перцем.", "Намазать на тост, выложить помидор.", "Сверху яйцо. Сбрызнуть маслом, посыпать чили."], "masha": "1 тост (~265 ккал, 10г белка). Для Фазы 1 добавить ещё 1 яйцо.", "tip": "Лимонный сок не даёт авокадо темнеть — готовить перед едой.", "why": ""}, {"id": "b8", "ch": "breakfast", "name": "Тост с арахисовой пастой и фруктами", "time": 5, "tags": ["до 20 мин"], "kbju": {"k": 255, "p": 8, "f": 10, "c": 33}, "ing": ["Хлеб цельнозерновой — 2 куска", "Арахисовая паста 100% — 1,5 ст.л. (25г)", "Банан — ½ шт или груша", "Семена чиа — 1 ч.л.", "Корица — ½ ч.л.", "Мёд — ½ ч.л. (по желанию)"], "steps": ["Поджарить хлеб.", "Намазать арахисовую пасту.", "Выложить нарезанный банан/грушу.", "Посыпать корицей и чиа."], "masha": "В дни без тренировок уменьшить пасту до 1 ч.л. (~320 ккал).", "tip": "Паста 100% — без сахара и масла. Корица улучшает усвоение углеводов.", "why": ""}, {"id": "b9", "ch": "breakfast", "name": "Творог со сметаной, ягодами и семенами", "time": 3, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 140, "p": 9, "f": 7, "c": 10}, "ing": ["Творог 5% — 150г", "Сметана 15% — 1 ст.л.", "Ягоды — горсть (80г)", "Семена чиа — 1 ч.л.", "Семена льна молотые — 1 ч.л.", "Овсяные отруби — 1 ст.л.", "Грецкие орехи — 15г", "Варенье/мёд — 1 ч.л. (по желанию)"], "steps": ["Творог смешать со сметаной.", "Сверху ягоды, чиа, лён, отруби.", "Посыпать дроблёными орехами.", "Добавить варенье по вкусу."], "masha": "Основной завтрак Маши. ~395 ккал, 26г белка — хорошо для Фазы 1.", "tip": "Смешать заранее и убрать в холодильник — к утру нежнее.", "why": "Семена льна и отруби — растворимая клетчатка, мягко помогает ЖКТ."}, {"id": "b10", "ch": "breakfast", "name": "Домашняя гранола", "time": 30, "tags": ["заготовка"], "kbju": {"k": 440, "p": 11, "f": 22, "c": 50}, "ing": ["Овсяные хлопья — 200г", "Грецкие орехи — 30г", "Миндаль — 30г", "Кешью — 20г", "Тыквенные семечки — 2 ст.л.", "Кунжут — 1 ст.л.", "Семена льна — 1 ст.л.", "Оливковое масло — 2 ст.л.", "Мёд/варенье — 2 ст.л.", "Корица — 1 ч.л.", "Чернослив/вишня — 50г (после запекания)"], "steps": ["Орехи порубить крупно.", "Смешать хлопья, орехи, семена с маслом, мёдом, корицей.", "Выложить слоем ~1 см на противень.", "Запекать 160°С 20–25 мин, мешать каждые 10 мин.", "Полностью остудить (нельзя убирать горячей!).", "Добавить сухофрукты. Хранить до 2 недель."], "masha": "50г гранолы + 150г йогурта + ягоды = ~380 ккал, 16г белка.", "tip": "Ключ к хрусту — полностью остудить.", "why": ""}, {"id": "b11", "ch": "breakfast", "name": "Рисовая каша с кокосовым молоком и манго", "time": 20, "tags": ["до 20 мин"], "kbju": {"k": 175, "p": 3, "f": 7, "c": 25}, "ing": ["Рис жасминовый — 80г", "Кокосовое молоко — 150мл", "Вода — 150мл", "Манго — ½ шт", "Корица — ½ ч.л.", "Семена чиа — 1 ч.л.", "Соль — щепотка"], "steps": ["Смешать кокосовое молоко и воду, довести до кипения.", "Добавить рис и соль. Варить 15–20 мин на малом огне, помешивая.", "Добавить корицу.", "Выложить в миску, сверху манго и чиа."], "masha": "~360 ккал. В тренировочный день добавить 2 варёных яйца (+14г белка).", "tip": "Мешать часто — рис с кокосовым молоком пристаёт ко дну.", "why": ""}, {"id": "b12", "ch": "breakfast", "name": "Смузи-боул с гранолой", "time": 5, "tags": ["до 20 мин"], "kbju": {"k": 130, "p": 4, "f": 4, "c": 20}, "ing": ["Черника замороженная — 100г", "Вишня замороженная — 50г", "Банан замороженный — ½ шт", "Кефир/йогурт — 100мл", "Гранола — 30г", "Семена чиа — 1 ч.л."], "steps": ["Пробить замороженные ягоды и банан с кефиром.", "Консистенция густая, как мягкое мороженое.", "Переложить в миску.", "Сверху гранола и чиа. Есть сразу."], "masha": "~450 ккал, 14г белка. Углеводный завтрак — хорошо в день тренировки.", "tip": "Есть сразу — гранола размягчается через 5–7 мин.", "why": ""}, {"id": "b13", "ch": "breakfast", "name": "Банановые панкейки (3 ингредиента)", "time": 15, "tags": ["до 20 мин"], "kbju": {"k": 145, "p": 6, "f": 4, "c": 22}, "ing": ["Банан — 1 шт (120г), спелый", "Яйца — 2 шт", "Рисовая/овсяная мука — 3 ст.л. (45г)"], "steps": ["Банан измельчить в блендере с яйцами.", "Добавить муку, перемешать.", "Жарить небольшие панкейки по 2–3 мин с каждой стороны.", "Получается 6–8 шт."], "masha": "4 панкейка — 275 ккал, 11г белка. В тренировочный день добавить йогурт.", "tip": "Банан очень спелый — даёт сладость и связывает тесто.", "why": ""}, {"id": "b14", "ch": "breakfast", "name": "Чиа пудинг", "time": 5, "tags": ["до 20 мин", "ЖКТ", "заготовка"], "kbju": {"k": 120, "p": 4, "f": 7, "c": 11}, "ing": ["Семена чиа — 5–6 ст.л. (60г)", "Кокосовое молоко — 200мл", "Вода — 200мл", "Гранола — 50г (для подачи)", "Ягоды — 80г", "Банан — ½ шт"], "steps": ["Смешать чиа с молоком и водой.", "Через 2 мин перемешать снова — семена не должны осесть.", "Повторить 1–2 раза. В холодильник на ночь.", "Утром добавить гранолу, ягоды, банан."], "masha": "420 ккал, 12г белка, богат клетчаткой и омега-3 — отлично для ЖКТ. Готовить вечером.", "tip": "Перемешивать несколько раз в первые 15 мин — иначе слипнутся.", "why": "Чиа — растворимая клетчатка + омега-3: мягкий стул и противовоспалительный эффект."}, {"id": "b15", "ch": "breakfast", "name": "Зелёный протеиновый смузи", "time": 5, "tags": ["до 20 мин"], "kbju": {"k": 55, "p": 3, "f": 1, "c": 8}, "ing": ["Шпинат свежий — горсть (80г)", "Банан замороженный — ½ шт", "Кефир/йогурт — 200мл", "Семена чиа — 1 ч.л.", "Имбирь тёртый — ½ ч.л.", "Лимонный сок — 1 ч.л.", "Мёд — ½ ч.л. (по желанию)"], "steps": ["Пробить шпинат с кефиром до однородности.", "Добавить банан, имбирь, чиа, лимон, мёд.", "Пробить ещё 30 сек.", "Пить сразу."], "masha": "~200 ккал, 10г белка. Быстрый завтрак перед тренировкой.", "tip": "Шпинат пробивать первым — исчезнет во вкусе.", "why": ""}, {"id": "b16", "ch": "breakfast", "name": "Омлет со шпинатом и Фетой", "time": 7, "tags": ["до 20 мин", "белок"], "kbju": {"k": 155, "p": 12, "f": 11, "c": 2}, "ing": ["Яйца — 2 шт", "Шпинат свежий — горсть (60г)", "Фета — 30г", "Масло сливочное — ½ ч.л.", "Соль, перец", "По желанию: черри 4–5 шт"], "steps": ["Взбить яйца с солью и перцем.", "Шпинат обжарить на масле 1–2 мин.", "Вылить яйца. Не мешать — ждать пока края схватятся.", "Раскрошить Фету на половину.", "Сложить пополам, накрыть крышкой на 1 мин."], "masha": "Весь омлет — 415 ккал, 32г белка. Сытный ужин.", "tip": "Не мешать — это не scrambled. Складывать когда центр влажный.", "why": ""}, {"id": "b17", "ch": "breakfast", "name": "Запечённые помидоры с Фетой и яйцами", "time": 20, "tags": ["до 20 мин"], "kbju": {"k": 85, "p": 5, "f": 5, "c": 6}, "ing": ["Помидоры крупные — 4 шт", "Фета — 60г", "Яйца — 2 шт", "Оливковое масло — 1 ч.л.", "Орегано — щепотка", "Соль, перец", "Базилик (по желанию)"], "steps": ["Разогреть духовку до 200°С.", "Срезать верхушки помидоров, вынуть часть мякоти.", "Посолить внутри, добавить каплю масла.", "В каждый — Фета и орегано. В 2 крупных — по яйцу.", "Духовка 200°С 15–18 мин."], "masha": "390 ккал, 23г белка. Красивый завтрак или лёгкий ужин.", "tip": "Мякоть не выбрасывать — в соус или суп.", "why": ""}, {"id": "b19", "ch": "breakfast", "name": "Творожные сырники", "time": 20, "tags": ["до 20 мин", "белок"], "kbju": {"k": 175, "p": 13, "f": 5, "c": 18}, "ing": ["Творог 5% — 300г", "Яйца — 2 шт", "Мука/молотые хлопья — 3 ст.л.", "Сахар — 1 ст.л. (или без)", "Ваниль, соль — щепотка", "Мука для обваливания — 2 ст.л."], "steps": ["Смешать творог, яйца, муку, ваниль, соль.", "Если масса влажная — добавить ложку муки.", "Сформировать сырники, обвалять в муке.", "Аэрогриль 180°С 10–12 мин, или духовка 180°С 20–25 мин, или сковорода 3–4 мин/сторону на малом огне."], "masha": "3–4 сырника (~395 ккал, 29г белка). Хороший завтрак в тренировочный день.", "tip": "Влажный творог отжать через марлю. На малом огне — не подгорят.", "why": ""}, {"id": "b20", "ch": "breakfast", "name": "Омлет со шпинатом и Камамбером", "time": 8, "tags": ["до 20 мин", "белок"], "kbju": {"k": 190, "p": 12, "f": 15, "c": 2}, "ing": ["Яйца — 2 шт", "Шпинат свежий — горсть (60г)", "Камамбер — 30г", "Масло сливочное — ½ ч.л.", "Соль, перец"], "steps": ["Взбить яйца с солью.", "Шпинат обжарить на масле 1–2 мин.", "Вылить яйца, не мешать.", "Выложить камамбер на половину.", "Сложить, накрыть крышкой на 1 мин."], "masha": "Весь омлет — 510 ккал, 32г белка. Насыщенный ужин.", "tip": "Камамбер расплавится под крышкой.", "why": ""}, {"id": "b21", "ch": "breakfast", "name": "Тост с Моцареллой, помидором и авокадо", "time": 7, "tags": ["до 20 мин"], "kbju": {"k": 195, "p": 8, "f": 13, "c": 13}, "ing": ["Хлеб цельнозерновой — 2 куска", "Моцарелла — 50г", "Помидор — 1 шт", "Авокадо — ½ шт", "Оливковое масло — 1 ч.л.", "Лимонный сок, соль, базилик"], "steps": ["Хлеб поджарить.", "Авокадо размять с лимоном и солью.", "Намазать на тост, выложить помидор, сверху моцарелла.", "Сбрызнуть маслом, посыпать базиликом."], "masha": "1 тост — 285 ккал, 12г белка.", "tip": "Моцарелла комнатной температуры вкуснее.", "why": ""}, {"id": "b24", "ch": "breakfast", "name": "Овсяная каша с ягодами и семенами", "time": 10, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 140, "p": 5, "f": 4, "c": 21}, "ing": ["Овсяные хлопья — 80г", "Молоко/кокосовое — 200мл", "Ягоды замороженные — горсть (80г)", "Семена чиа — 1 ч.л.", "Семена льна молотые — 1 ч.л.", "Корица — ½ ч.л.", "Мёд — 1 ч.л. (по желанию)", "Соль — щепотка"], "steps": ["Хлопья залить молоком, добавить соль.", "Варить 5–7 мин, помешивая.", "Добавить корицу.", "Сверху ягоды, семена, мёд."], "masha": "~430 ккал, 16г белка. В Фазе 2 добавить горсть орехов.", "tip": "Ягоды добавлять замороженными — дадут сок прямо в каше.", "why": "Овёс — бета-глюкан (растворимая клетчатка): сытость и мягкий стул."}, {"id": "b26", "ch": "breakfast", "name": "Зернёный творог Маши (с семенами и черносливом)", "time": 2, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 115, "p": 11, "f": 3, "c": 11}, "ing": ["Зернёный творог 5% — 150–170г", "Варенье — 1 ч.л.", "Семена чиа — 1 ч.л.", "Семена льна молотые — 1 ч.л.", "Чернослив — 3 шт"], "steps": ["Выложить творог в миску.", "Добавить варенье, сверху чиа и лён.", "Положить чернослив рядом."], "masha": "~215г = 250 ккал, 24г белка. Лёгкий ужин с клетчаткой — идеально для Фазы 1.", "tip": "Чернослив ежедневно — для регулярности ЖКТ.", "why": "Чернослив (сорбитол) + молотый лён — доказанная мягкая помощь при запоре."}, {"id": "s1", "ch": "soup", "name": "Куриный суп классический", "time": 50, "tags": [], "kbju": {"k": 45, "p": 5, "f": 1, "c": 4}, "ing": ["Курица (грудка/бёдра) — 400–500г", "Картошка — 2 шт", "Морковь — 1 шт", "Лук — 1 шт", "Рис 3 ст.л. или вермишель 50г", "Чеснок — 2 зубчика", "Лавровый лист, перец горошком", "Соль", "Вода — 2л"], "steps": ["Курицу залить 2л холодной воды, довести до кипения, снять пену.", "Добавить целую луковицу, морковь, лавровый лист, перец. Варить 30 мин.", "Вынуть курицу и луковицу (лук выбросить).", "Добавить картошку кубиком и рис/вермишель.", "Варить 15 мин.", "Курицу разобрать, вернуть. Добавить чеснок. Посолить."], "masha": "300мл + 120г курицы — 235 ккал, 26г белка. Хороший лёгкий обед.", "tip": "Прозрачный бульон — от медленного кипения и снятия пены.", "why": ""}, {"id": "s2", "ch": "soup", "name": "Кокосовый крем-суп с курицей", "time": 35, "tags": [], "kbju": {"k": 90, "p": 8, "f": 5, "c": 4}, "ing": ["Куриная грудка — 400г", "Цветная капуста — 300г", "Кокосовое молоко — 200мл", "Вода — 500мл", "Лук — 1 шт", "Морковь — 1 шт", "Имбирь — 2 см", "Чеснок — 3 зубчика", "Оливковое масло — 1 ст.л.", "Куркума — ½ ч.л.", "Соль, перец"], "steps": ["Лук и морковь обжарить на масле 5 мин.", "Добавить имбирь и чеснок, 2 мин.", "Добавить курицу кубиками и капусту. Залить водой, варить 20 мин.", "Достать курицу.", "Влить кокосовое молоко, пробить блендером.", "Вернуть курицу, прогреть. Добавить куркуму, посолить."], "masha": "350мл + 80г курицы — 285 ккал, 28г белка. Сытный обед при небольшом калораже.", "tip": "Кокосовое молоко добавлять после блендера — не кипятить.", "why": ""}, {"id": "s3", "ch": "soup", "name": "Крем-суп из брокколи с Пармезаном", "time": 25, "tags": [], "kbju": {"k": 65, "p": 4, "f": 3, "c": 5}, "ing": ["Брокколи — 500г", "Лук — 1 шт", "Чеснок — 2 зубчика", "Бульон/вода — 600мл", "Пармезан — 30г", "Оливковое масло — 1 ст.л.", "Мускатный орех — щепотка", "Соль, перец"], "steps": ["Лук обжарить на масле 3 мин, добавить чеснок 1 мин.", "Добавить брокколи и бульон. Довести до кипения.", "Варить 12–15 мин до мягкости.", "Пробить блендером.", "Добавить пармезан, мускат, соль, перец."], "masha": "Та же порция — 320 ккал. Хороший обед.", "tip": "Мускатный орех обязателен — раскрывает вкус брокколи.", "why": ""}, {"id": "s4", "ch": "soup", "name": "Суп с фрикадельками", "time": 40, "tags": [], "kbju": {"k": 75, "p": 6, "f": 3, "c": 5}, "ing": ["Говяжий фарш — 300г", "Яйцо — 1 шт", "Лук — 1 шт", "Картошка — 2 шт", "Морковь — 1 шт", "Лавровый лист", "Зелень", "Соль, перец", "Вода — 1,5л"], "steps": ["½ луковицы смешать с фаршем, яйцом, солью, перцем.", "Скатать фрикадельки с грецкий орех.", "Остальной лук и морковь обжарить 5 мин.", "Залить водой, добавить картошку. Через 10 мин — фрикадельки.", "Не мешать 3 мин. Варить 15 мин. Добавить лавровый лист, зелень."], "masha": "Та же порция — 290 ккал, 22г белка. Сытный обед.", "tip": "Тёртый лук в фарш — фрикадельки держат форму.", "why": ""}, {"id": "s5", "ch": "soup", "name": "Тыквенный крем-суп", "time": 30, "tags": [], "kbju": {"k": 75, "p": 2, "f": 4, "c": 8}, "ing": ["Тыква — 600г", "Лук — 1 шт", "Чеснок — 3 зубчика", "Кокосовое молоко — 150мл", "Вода — 400мл", "Имбирь — 1 см", "Куркума — ½ ч.л.", "Мускатный орех — щепотка", "Оливковое масло — 1 ст.л.", "Соль, перец"], "steps": ["Лук и чеснок обжарить 3 мин.", "Добавить тыкву, имбирь, куркуму.", "Залить водой, варить 20 мин.", "Влить кокосовое молоко, пробить блендером.", "Посолить, добавить мускат."], "masha": "350мл — 265 ккал. Добавить 100г куриной грудки для полноценного обеда (+22г белка).", "tip": "Тыква Хокайдо не требует чистки.", "why": ""}, {"id": "s6", "ch": "soup", "name": "Красный чечевичный суп (Мерджимек)", "time": 30, "tags": ["ЖКТ", "железо"], "kbju": {"k": 80, "p": 5, "f": 2, "c": 12}, "ing": ["Красная чечевица — 200г", "Лук — 1 шт", "Морковь — 1 шт", "Чеснок — 3 зубчика", "Томатная паста — 1 ст.л.", "Куркума, тмин, паприка", "Оливковое масло — 1 ст.л.", "Вода/бульон — 1л", "Лимонный сок — 1 ст.л.", "Соль, перец"], "steps": ["Лук и морковь обжарить 5 мин.", "Добавить чеснок, специи, томатную пасту — 2 мин.", "Добавить чечевицу и воду. Варить 20 мин.", "Пробить блендером. Посолить. Лимонный сок при подаче."], "masha": "350мл — 310 ккал, 20г белка. Растительный белок + клетчатка = отлично для ЖКТ.", "tip": "Лимонный сок в конце обязателен. Чечевица варится без замачивания.", "why": "Чечевица — негемовое железо + клетчатка; витамин C из лимона улучшает усвоение железа."}, {"id": "f4", "ch": "fish", "name": "Тунец с хрустящей корочкой", "time": 10, "tags": ["до 20 мин", "белок"], "kbju": {"k": 145, "p": 24, "f": 5, "c": 0}, "ing": ["Стейк тунца — 160г", "Оливковое масло — 1 ч.л.", "Соль, перец", "Кунжут — 1 ст.л. (по желанию)"], "steps": ["Стейк обсушить — критично для корочки.", "Посолить, поперчить (или обвалять в кунжуте).", "Сковороду раскалить на максимуме.", "Выложить тунца, не трогать 2 мин.", "Перевернуть, ещё 1,5–2 мин.", "Дать отдохнуть 1 мин."], "masha": "1 стейк 250г = 360 ккал, 60г белка. Тунец — 1–2 раза в неделю (ртуть), особенно если возможна беременность.", "tip": "Внутри розовый центр — это правильно. Пересушенный тунец резиновый.", "why": ""}, {"id": "f5", "ch": "fish", "name": "Тунец запечённый в фольге с лимоном", "time": 20, "tags": ["до 20 мин", "белок"], "kbju": {"k": 135, "p": 23, "f": 4, "c": 0}, "ing": ["Стейк тунца — 160г", "Лимон — ½ шт", "Оливковое масло — 1 ч.л.", "Тимьян/розмарин — щепотка", "Соль, перец"], "steps": ["Стейк на фольгу, посолить, сбрызнуть маслом.", "Сверху лимон и тимьян.", "Завернуть конвертом плотно.", "Духовка 180°С 15 мин (аэрогриль 180°С 12 мин)."], "masha": "1 стейк 250г = 340 ккал, 58г белка. Тунец — 1–2 раза в неделю из-за ртути.", "tip": "Не передерживать — 15 мин достаточно.", "why": ""}, {"id": "f7", "ch": "fish", "name": "Лосось с кунжутной корочкой", "time": 15, "tags": ["до 20 мин", "белок", "омега"], "kbju": {"k": 220, "p": 24, "f": 13, "c": 2}, "ing": ["Филе лосося — 150–180г", "Кунжут — 2 ст.л.", "Горчица зернистая — 1 ч.л.", "Соль, перец"], "steps": ["Лосось обсушить, посолить верх.", "Смазать горчицей, обвалять в кунжуте.", "Жарить на сухой раскалённой сковороде кунжутом вниз 3 мин.", "Перевернуть, ещё 3–4 мин. Внутри чуть розовый."], "masha": "200г в Фазе 1 (~440 ккал, 48г белка).", "tip": "Горчица держит кунжут. Сковорода полностью сухая и горячая.", "why": "Жирная рыба — омега-3: противовоспалительно, поддержка настроения."}, {"id": "f8", "ch": "fish", "name": "Лосось запечённый в фольге с лимоном", "time": 20, "tags": ["до 20 мин", "белок", "омега"], "kbju": {"k": 195, "p": 22, "f": 11, "c": 0}, "ing": ["Филе лосося — 150–180г", "Лимон — ½ шт", "Оливковое масло — 1 ч.л.", "Тимьян/укроп — щепотка", "Соль, перец"], "steps": ["Порцию завернуть в фольгу.", "Посолить, сбрызнуть маслом и лимоном.", "Сверху лимон и зелень. Завернуть конвертом.", "Духовка 180°С 18 мин (аэрогриль 180°С 14–15 мин)."], "masha": "200г в Фазе 1 (~390 ккал, 44г белка).", "tip": "Открывать осторожно — горячий пар.", "why": "Омега-3 2 раза в неделю — доказанная польза для настроения и сердца."}, {"id": "f11", "ch": "fish", "name": "Паста с лососем и шпинатом", "time": 20, "tags": ["до 20 мин", "омега"], "kbju": {"k": 195, "p": 12, "f": 9, "c": 18}, "ing": ["Паста — 65–80г", "Лосось — 100–120г", "Шпинат — горсть (60г)", "Сливки 20% — 100мл", "Чеснок — 2 зубчика", "Оливковое масло — 1 ч.л.", "Цедра лимона", "Соль, белый перец, мускат"], "steps": ["Отварить пасту al dente, сохранить воду.", "Обжарить чеснок 30 сек, добавить лосось — 2 мин.", "Добавить шпинат, влить сливки.", "Цедра, соль, перец, мускат. Тушить 2 мин.", "Добавить пасту, при необходимости воду от варки."], "masha": "65г пасты + 100г лосося = 480 ккал, 35г белка. Хорошо в тренировочный день.", "tip": "Цедра (не сок!) — аромат без кислоты.", "why": ""}, {"id": "m1", "ch": "meat", "name": "Говяжий гуляш (готовить накануне)", "time": 80, "tags": ["железо"], "kbju": {"k": 155, "p": 17, "f": 8, "c": 5}, "ing": ["Говяжья вырезка — 500г", "Лук — 2 шт", "Морковь — 1 шт", "Чеснок — 3 зубчика", "Помидоры — 2 шт или паста 1 ст.л.", "Паприка — 1 ч.л.", "Лавровый лист", "Оливковое масло — 1,5 ст.л.", "Соль, перец", "Вода — ~400мл"], "steps": ["Мясо нарезать кубиками, обсушить.", "Обжарить партиями на максимуме, не мешать 2 мин. Убрать.", "Обжарить лук до золотистого (5–7 мин).", "Добавить морковь, чеснок, паприку — 3 мин.", "Вернуть мясо, добавить помидоры.", "Залить водой, тушить 1 час под крышкой.", "Лавровый лист, соль в конце."], "masha": "170г порция — 365 ккал, 39г белка. С гречкой 80г — обед Фазы 2.", "tip": "Вкуснее на следующий день. Партионная жарка обязательна.", "why": "Красное мясо — гемовое железо (усваивается лучше всего)."}, {"id": "m3", "ch": "meat", "name": "Стейк говяжий", "time": 15, "tags": ["до 20 мин", "белок", "железо"], "kbju": {"k": 270, "p": 28, "f": 17, "c": 0}, "ing": ["Стейк говяжий — 160–200г", "Оливковое масло — 1 ч.л.", "Соль крупная, перец"], "steps": ["Разморозить накануне. За 30 мин до жарки вынуть из холодильника.", "Обсушить, посолить, поперчить, смазать маслом.", "Сковороду раскалить на максимуме.", "Выложить стейк, не двигать 3–4 мин.", "Перевернуть, ещё 3–4 мин (medium 57–63°С).", "Дать отдохнуть 5 мин."], "masha": "160г = 430 ккал, 45г белка. Отличный вариант в день тренировки.", "tip": "Не двигать стейк — корочка только при постоянном контакте.", "why": ""}, {"id": "m4", "ch": "meat", "name": "Говяжьи котлеты в духовке", "time": 30, "tags": ["железо"], "kbju": {"k": 205, "p": 20, "f": 13, "c": 3}, "ing": ["Говяжий фарш — 400г", "Лук — 1 шт (натереть)", "Яйцо — 1 шт", "Чеснок — 2 зубчика", "Соль, перец, паприка", "Горчица — 1 ч.л."], "steps": ["Лук натереть на мелкой тёрке.", "Смешать фарш, лук, яйцо, чеснок, специи.", "Убрать в холодильник на 15 мин.", "Сформировать котлеты толщиной 2 см.", "Духовка 200°С 22–25 мин (аэрогриль 190°С 15–17 мин)."], "masha": "1,5 котлеты (~150г) — 310 ккал, 30г белка.", "tip": "Тёртый лук — секрет сочных котлет.", "why": ""}, {"id": "c1", "ch": "chicken", "name": "Печёночные оладьи из куриной печени", "time": 25, "tags": ["железо"], "kbju": {"k": 165, "p": 17, "f": 9, "c": 5}, "ing": ["Куриная печень — 400г", "Лук — 1 шт", "Яйца — 2 шт", "Кукурузная/гречневая мука — 2 ст.л.", "Соль, перец", "Масло для жарки — 2 ст.л."], "steps": ["Печень и лук пробить блендером.", "Добавить яйца, муку, соль, перец.", "Выкладывать ложкой на горячую сковороду (~8 см).", "Жарить 3–4 мин с каждой стороны."], "masha": "3 оладьи — 295 ккал, 31г белка. Важный источник железа.", "tip": "Печень — рекордсмен по железу, B12 и фолиевой кислоте.", "why": "Куриная печень 1–2 раза в неделю — самый эффективный пищевой источник железа и B12."}, {"id": "c2", "ch": "chicken", "name": "Пряная куриная грудка (мариновать накануне)", "time": 25, "tags": ["белок"], "kbju": {"k": 135, "p": 26, "f": 3, "c": 2}, "ing": ["Куриная грудка — 2 шт (~400г)", "Кефир 2,5% — 3 ст.л.", "Чеснок — 2 зубчика", "Паприка — 1 ч.л.", "Горчица зернистая — 1 ч.л.", "Оливковое масло — 1 ч.л.", "Соль, перец"], "steps": ["Грудку отбить до толщины ~2 см.", "Смешать кефир, чеснок, паприку, горчицу, масло, соль.", "Обмазать, убрать в холодильник на ночь.", "Достать за 20 мин до готовки.", "Духовка 180°С 20–22 мин (аэрогриль 180°С 18 мин).", "Дать отдохнуть 5 мин."], "masha": "150–170г = 230 ккал, 39г белка. Отличный белок при низком калораже.", "tip": "Кефир — секрет сочности: кислота расщепляет белок.", "why": ""}, {"id": "c3", "ch": "chicken", "name": "Куриная грудка по-азиатски с кунжутом", "time": 20, "tags": ["до 20 мин", "белок"], "kbju": {"k": 150, "p": 27, "f": 4, "c": 2}, "ing": ["Куриная грудка — 2 шт (~400г)", "Соевый соус — 2 ст.л.", "Имбирь тёртый — 1 ч.л.", "Чеснок — 2 зубчика", "Кунжут — 1 ст.л.", "Оливковое масло — 1 ч.л."], "steps": ["Грудку нарезать полосками 1,5 см.", "Смешать соус, имбирь, чеснок. Замариновать 15 мин.", "Обжарить в один слой 3–4 мин без перемешивания, затем 2–3 мин помешивая.", "Посыпать кунжутом."], "masha": "150–170г = 255 ккал, 41г белка.", "tip": "Соевый соус солёный — не досаливать. Резать поперёк волокон.", "why": ""}, {"id": "c8", "ch": "chicken", "name": "Куриная грудка в духовке — заготовка на неделю", "time": 45, "tags": ["заготовка", "белок"], "kbju": {"k": 130, "p": 26, "f": 3, "c": 0}, "ing": ["Куриная грудка — 4–6 шт (800г–1,2кг)", "Соль, перец", "Чеснок — 3 зубчика", "Оливковое масло — 1 ст.л.", "Паприка — ½ ч.л."], "steps": ["Грудки обсушить, натереть маслом, чесноком, специями.", "Выложить в форму, накрыть фольгой плотно.", "Духовка 150°С 45 мин.", "Не открывать 15 мин после выключения.", "Остудить, убрать в холодильник. Хранить до 4 дней."], "masha": "170г = 215 ккал, 39г белка. Самая удобная заготовка — обед за 5 мин.", "tip": "150°С + фольга = всегда сочная грудка.", "why": ""}, {"id": "c9", "ch": "chicken", "name": "Куриная печень с луком и яблоком", "time": 20, "tags": ["до 20 мин", "железо"], "kbju": {"k": 145, "p": 17, "f": 6, "c": 7}, "ing": ["Куриная печень — 400г", "Лук — 2 шт", "Яблоко кислое — 1 шт", "Масло сливочное — 1 ч.л.", "Оливковое масло — 1 ст.л.", "Соль, перец, тимьян"], "steps": ["Печень очистить от плёнок, нарезать.", "Лук обжарить на смеси масел 7–8 мин.", "Добавить яблоко, тимьян. Жарить 3 мин.", "Печень одним слоем 2–3 мин без перемешивания.", "Перевернуть, ещё 2 мин. Солить в конце.", "Центр чуть розовый."], "masha": "150г = 275 ккал, 32г белка + железо + B12. 1–2 раза в неделю.", "tip": "Соль только в конце — иначе печень жёсткая. Яблоко убирает горечь.", "why": "Печень + лук (витамин C) — классическое сочетание для усвоения железа."}, {"id": "c11", "ch": "chicken", "name": "Паштет из куриной печени", "time": 25, "tags": ["заготовка", "железо"], "kbju": {"k": 210, "p": 15, "f": 15, "c": 3}, "ing": ["Куриная печень — 400г", "Лук — 1 шт", "Чеснок — 3 зубчика", "Масло сливочное — 40г + 20г для верха", "Коньяк — 2 ст.л. (по желанию)", "Тимьян, соль, перец"], "steps": ["Лук и чеснок обжарить на 20г масла 5 мин.", "Добавить печень, жарить 4–5 мин.", "Влить коньяк, выпарить 1 мин.", "Пробить блендером с 20г масла до гладкости.", "Переложить в форму, залить растопленным маслом.", "В холодильник на 2 часа. Хранить до 5 дней."], "masha": "2 ст.л. = 63 ккал, 5г белка. Перекус на хлебе, богат железом и B12.", "tip": "Масляный слой сверху — паштет не темнеет.", "why": ""}, {"id": "sal2", "ch": "salad", "name": "Тёплый салат с нутом, шпинатом и Фетой", "time": 15, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 170, "p": 7, "f": 10, "c": 14}, "ing": ["Нут консервированный — 200г", "Шпинат свежий — горсть (80г)", "Фета — 50г", "Чеснок — 2 зубчика", "Оливковое масло — 2 ст.л.", "Лимонный сок — 1 ст.л.", "Куркума — ½ ч.л.", "Соль, перец"], "steps": ["Нут обсушить.", "Обжарить с чесноком и куркумой 3–4 мин.", "Добавить шпинат — увянет за 1 мин.", "Снять с огня, добавить лимон и масло.", "Раскрошить Фету. Подавать тёплым."], "masha": "200г = 340 ккал, 14г белка. Сытный ужин или обед.", "tip": "Горячий нут с холодной Фетой — контраст делает салат особенным.", "why": "Нут — клетчатка + растительный белок, мягко для ЖКТ."}, {"id": "sal3", "ch": "salad", "name": "Греческий салат с курицей", "time": 15, "tags": ["до 20 мин", "белок"], "kbju": {"k": 115, "p": 11, "f": 7, "c": 4}, "ing": ["Куриная грудка — 150г", "Огурец — 1 шт", "Помидоры — 2 шт", "Фета — 50г", "Маслины — горсть", "Лук красный — ½ шт", "Оливковое масло — 1,5 ст.л.", "Орегано", "Соль, перец"], "steps": ["Огурец, помидоры, лук нарезать крупно.", "Курицу нарезать полосками.", "Смешать, добавить Фету и маслины.", "Заправить: масло + орегано + соль."], "masha": "300г = 345 ккал, 33г белка. Полноценный обед без гарнира.", "tip": "Нарезать крупно. Заправлять перед подачей.", "why": ""}, {"id": "sal7", "ch": "salad", "name": "Салат Нисуаз с тунцом", "time": 15, "tags": ["до 20 мин", "белок"], "kbju": {"k": 120, "p": 12, "f": 7, "c": 4}, "ing": ["Тунец консерв. — 160г или свежий", "Яйца варёные — 2 шт", "Зелёная фасоль — 80г", "Помидоры черри — 8 шт", "Огурец — 1 шт", "Маслины — горсть", "Листья салата", "Заправка: масло 2 ст.л. + лимон 1 ст.л. + горчица ½ ч.л."], "steps": ["Фасоль бланшировать 3 мин, в холодную воду.", "Яйца на четвертинки, тунец слить.", "Выложить на салат. Заправить перед подачей."], "masha": "Весь салат — 440 ккал, 44г белка. Отличный обед.", "tip": "Нисуаз не перемешивают — выкладывают по зонам.", "why": ""}, {"id": "v3", "ch": "veg", "name": "Тушёный шпинат с чесноком", "time": 5, "tags": ["до 20 мин"], "kbju": {"k": 40, "p": 3, "f": 2, "c": 3}, "ing": ["Шпинат свежий — 100–150г", "Чеснок — 2 зубчика", "Оливковое масло — 1 ч.л.", "Соль, мускатный орех — щепотка"], "steps": ["Разогреть масло.", "Добавить чеснок, 30 сек.", "Добавить шпинат, жарить 1–2 мин.", "Посолить, добавить мускат."], "masha": "80г готового — 50 ккал. Почти без калорий — добавлять к любому блюду.", "tip": "Шпинат уменьшается в 5–6 раз.", "why": ""}, {"id": "v1", "ch": "veg", "name": "Брокколи паровая", "time": 8, "tags": ["до 20 мин"], "kbju": {"k": 45, "p": 3, "f": 1, "c": 5}, "ing": ["Брокколи — 150–200г", "Соль — щепотка", "Оливковое масло — 1 ч.л.", "Лимонный сок — несколько капель"], "steps": ["Разобрать на соцветия.", "В кастрюлю 3 см воды, довести до кипения.", "Брокколи в пароварку/дуршлаг над водой.", "Накрыть, готовить 5–6 мин — ярко-зелёная, чуть хрустит.", "Сбрызнуть маслом и лимоном."], "masha": "150г = 75 ккал, 5г белка. Почти бесплатный гарнир.", "tip": "Переложить в холодную воду после варки — останется яркой.", "why": ""}, {"id": "d1", "ch": "baking", "name": "Овсяное печенье без муки и сахара", "time": 20, "tags": ["до 20 мин"], "kbju": {"k": 345, "p": 8, "f": 10, "c": 55}, "ing": ["Овсяные хлопья — 150г", "Банан спелый — 2 шт", "Арахисовая паста — 2 ст.л.", "Корица — 1 ч.л.", "Ваниль — щепотка", "По желанию: шоколад/вишня — 30г"], "steps": ["Бананы размять в пюре.", "Смешать с пастой, хлопьями, корицей, ванилью.", "Дать постоять 5 мин.", "Выложить ложкой на бумагу (12–14 шт).", "Духовка 170°С 12–15 мин.", "Остудить 5 мин."], "masha": "2 печенья = 275 ккал. Перекус или сладкое после тренировки.", "tip": "Спелые бананы с пятнами — для сладости.", "why": ""}, {"id": "d4", "ch": "baking", "name": "Шоколадный мусс из авокадо", "time": 10, "tags": ["до 20 мин"], "kbju": {"k": 185, "p": 2, "f": 15, "c": 12}, "ing": ["Авокадо спелый — 2 шт", "Какао-порошок — 3 ст.л.", "Кокосовое молоко — 3 ст.л.", "Мёд — 1 ст.л.", "Ваниль — ½ ч.л.", "Соль — щепотка"], "steps": ["Авокадо должен быть спелым.", "Пробить всё блендером до гладкости.", "Попробовать, добавить мёд/какао по вкусу.", "В холодильник на 30 мин.", "Подавать с ягодами."], "masha": "½ порции = 240 ккал. Полезные жиры, умеренные углеводы.", "tip": "Никто не угадывает авокадо — мусс как настоящий шоколадный.", "why": ""}, {"id": "d6", "ch": "baking", "name": "Запечённое яблоко с корицей и мёдом", "time": 15, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 85, "p": 1, "f": 2, "c": 16}, "ing": ["Яблоко — 1 шт (кисло-сладкое)", "Мёд — 1 ч.л.", "Корица — ½ ч.л.", "Масло сливочное — ½ ч.л.", "Грецкие орехи — 1 ч.л. (по желанию)"], "steps": ["Вырезать сердцевину снизу вверх, оставить донышко.", "Внутрь — масло, мёд, корицу, орехи.", "Аэрогриль 180°С 12–15 мин (духовка 180°С 20–25 мин).", "Готово когда кожица морщится, мякоть мягкая."], "masha": "1 яблоко — 170 ккал. Лёгкий десерт вместо конфет.", "tip": "Донышко оставлять целым — иначе начинка вытечет.", "why": "Печёное яблоко — мягкая клетчатка (пектин), бережно для ЖКТ."}, {"id": "sn1", "ch": "sauce", "name": "Гуакамоле домашний", "time": 10, "tags": ["до 20 мин"], "kbju": {"k": 160, "p": 2, "f": 15, "c": 6}, "ing": ["Авокадо — 2 шт", "Лимонный/лаймовый сок — 1 ст.л.", "Чеснок — 1 зубчик", "Помидор — 1 шт", "Соль, перец", "Кинза (по желанию)"], "steps": ["Авокадо размять вилкой, оставить кусочки.", "Добавить лимон, чеснок, помидор, соль.", "Перемешать, попробовать."], "masha": "3 ст.л. = 80 ккал. Добавка к тостам или рыбе.", "tip": "Лимон — вкус и защита от потемнения.", "why": ""}, {"id": "sn2", "ch": "sauce", "name": "Домашний хумус", "time": 15, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 200, "p": 8, "f": 11, "c": 17}, "ing": ["Нут консервированный — 400г (сохранить жидкость)", "Тахини — 2 ст.л.", "Лимон — сок (~40мл)", "Чеснок — 1–2 зубчика", "Оливковое масло — 2 ст.л.", "Соль", "Паприка, тмин"], "steps": ["Пробить нут с тахини, лимоном, чесноком, маслом.", "Добавлять аква-фабу по 1 ст.л. до нужной консистенции.", "Посолить, пробить ещё 2 мин.", "Подавать с маслом, паприкой, тмином."], "masha": "4 ст.л. = 160 ккал, 6г белка. Снек или намазка.", "tip": "Аква-фаба (жидкость от нута) — секрет кремовой текстуры.", "why": ""}, {"id": "sn5", "ch": "sauce", "name": "Энергетические шарики без выпечки", "time": 15, "tags": ["заготовка"], "kbju": {"k": 395, "p": 10, "f": 16, "c": 52}, "ing": ["Овсяные хлопья — 100г", "Арахисовая паста — 3 ст.л.", "Мёд — 2 ст.л.", "Семена чиа — 1 ст.л.", "Кунжут — 1 ст.л.", "Корица — ½ ч.л.", "Шоколад/вишня — 30г"], "steps": ["Смешать всё в миске.", "Если сухо — добавить мёд, если липко — хлопьев.", "Скатать шарики ~20г.", "Обвалять в кунжуте.", "В холодильник на 30 мин."], "masha": "2 шарика = 160 ккал. Перекус между приёмами.", "tip": "Хранить в холодильнике до 2 недель.", "why": ""}, {"id": "dr1", "ch": "drink", "name": "Золотое молоко (куркума латте)", "time": 7, "tags": ["до 20 мин"], "kbju": {"k": 45, "p": 2, "f": 2, "c": 5}, "ing": ["Молоко/растительное — 250мл", "Куркума — ½ ч.л.", "Корица — ¼ ч.л.", "Имбирь — ¼ ч.л.", "Чёрный перец — щепотка", "Мёд — ½ ч.л. (по желанию)", "Кокосовое масло — ½ ч.л. (по желанию)"], "steps": ["Налить молоко в кастрюльку.", "Добавить специи. Нагреть, не доводя до кипения.", "Когда появится пар (~70°С) — снять.", "Добавить мёд."], "masha": "250мл — 115 ккал. Противовоспалительный напиток вечером.", "tip": "Чёрный перец увеличивает усвоение куркумина. Не кипятить.", "why": ""}, {"id": "dr3", "ch": "drink", "name": "Чай с имбирём, лимоном и мёдом", "time": 8, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 10, "p": 0, "f": 0, "c": 2}, "ing": ["Имбирь свежий — 3 см", "Лимон — ½ шт", "Мёд — 1 ч.л.", "Вода — 300мл", "Перец горошком — 2–3 (по желанию)"], "steps": ["Имбирь нарезать кружочками.", "Залить водой, варить 5 мин.", "Снять, добавить лимонный сок.", "Настоять 2–3 мин. Добавить мёд в тёплый (не горячий)."], "masha": "Почти без калорий. Помогает пищеварению — хорошо для ЖКТ.", "tip": "Мёд в тёплый напиток — выше 60°С теряются ферменты.", "why": ""}, {"id": "dr8", "ch": "drink", "name": "Ягодный кефир с семенами (пробиотик)", "time": 3, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 60, "p": 3, "f": 2, "c": 8}, "ing": ["Кефир с пробиотиком — 250мл", "Черника/вишня замороженная — 80г", "Семена чиа — 1 ч.л.", "Семена льна молотые — 1 ч.л.", "Мёд — ½ ч.л. (по желанию)"], "steps": ["Пробить кефир с ягодами.", "Добавить чиа и лён, перемешать.", "Дать постоять 10 мин — семена набухнут."], "masha": "~340мл — 205 ккал, 10г белка. Лучший напиток для ЖКТ — утром или вечером.", "tip": "Пробиотики + пребиотики (клетчатка семян) в одном стакане.", "why": "Кефир (пробиотик) + клетчатка семян (пребиотик) — поддержка микрофлоры на фоне железа."}, {"id": "dr10", "ch": "drink", "name": "Ромашковый чай с лавандой и мёдом", "time": 7, "tags": ["до 20 мин", "сон"], "kbju": {"k": 5, "p": 0, "f": 0, "c": 1}, "ing": ["Ромашка сушёная — 2 ч.л. или 1 пакетик", "Лаванда пищевая — ½ ч.л.", "Кипяток — 300мл", "Мёд — 1 ч.л.", "Лимон — 2–3 кружочка"], "steps": ["Ромашку и лаванду в заварник.", "Залить кипятком 90°С.", "Настоять 5 мин под крышкой.", "Процедить. Добавить мёд и лимон."], "masha": "Почти без калорий. Успокаивает, помогает при вздутии — вечером.", "tip": "Лаванду брать только пищевую. Расслабляющий напиток перед сном.", "why": "Ромашка — мягкий расслабляющий эффект; хороший вечерний ритуал при тревоге."}, {"id": "ferm4", "ch": "ferment", "name": "Домашняя квашеная капуста", "time": 20, "tags": ["заготовка", "ЖКТ"], "kbju": {"k": 19, "p": 1, "f": 0, "c": 4}, "ing": ["Капуста белокочанная — 1кг", "Морковь — 1 шт", "Соль НЕ йодированная — 20г", "Тмин — 1 ч.л. (по желанию)", "Банка 1–2л"], "steps": ["Капусту нашинковать, морковь натереть.", "Смешать с солью, мять 5–7 мин до сока.", "Утрамбовать в банку, покрыть соком.", "Гнёт сверху. 3–5 дней при комнатной температуре.", "Каждый день прокалывать до дна.", "Убрать в холодильник."], "masha": "80г ежедневно для ЖКТ. Лучший натуральный пробиотик.", "tip": "Только не йодированная соль — йод убивает бактерии.", "why": "Живые лактобактерии — могут поддерживать микрофлору кишечника."}, {"id": "add1", "ch": "breakfast", "name": "Киви + йогурт на ночь (анти-запор)", "time": 3, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 75, "p": 4, "f": 2, "c": 12}, "ing": ["Киви спелые — 2 шт", "Греческий йогурт — 150г", "Семена чиа — 1 ч.л.", "Грецкие орехи — 10г (по желанию)"], "steps": ["Киви очистить, нарезать.", "Выложить на йогурт.", "Посыпать чиа и орехами.", "Съесть вечером или на завтрак."], "masha": "~190 ккал, 12г белка. Мягкая помощь при запоре.", "tip": "Спелые киви мягче и слаще.", "why": "Киви помогает регулярности стула (есть РКИ) — мягкая опция к черносливу."}, {"id": "add2", "ch": "salad", "name": "Салат со скумбрией, яйцом и свёклой", "time": 12, "tags": ["до 20 мин", "железо", "омега"], "kbju": {"k": 150, "p": 14, "f": 9, "c": 6}, "ing": ["Скумбрия запечёная/консерв. — 100г", "Яйца варёные — 2 шт", "Свёкла запечёная — 100г", "Руккола/шпинат — 60г", "Оливковое масло — 1 ч.л.", "Лимонный сок, соль, перец"], "steps": ["Свёклу и яйца нарезать.", "Скумбрию разобрать на кусочки.", "Выложить на рукколу.", "Заправить маслом и лимоном."], "masha": "~330 ккал, 28г белка. Обед: железо + омега-3 в одной тарелке.", "tip": "Скумбрия — недорогая жирная рыба.", "why": "Сочетает гемовое железо (яйца/рыба) и омега-3; зелень даёт фолат и витамин C для усвоения железа."}, {"id": "add3", "ch": "drink", "name": "Тёплый какао на молоке с магнием", "time": 6, "tags": ["до 20 мин", "сон"], "kbju": {"k": 90, "p": 5, "f": 3, "c": 12}, "ing": ["Молоко/растительное — 250мл", "Какао натуральное — 2 ч.л.", "Мёд — 1 ч.л.", "Корица — щепотка", "Соль — крошечная щепотка"], "steps": ["Какао размешать с 2 ст.л. холодного молока.", "Остальное молоко нагреть до ~70°С.", "Добавить какао-пасту, мёд, корицу, соль.", "Взбить венчиком 1 мин."], "masha": "250мл — 140 ккал, 8г белка. Вечерний напиток вместо десерта.", "tip": "Сначала какао с холодным молоком — иначе комочки.", "why": "Какао и молоко содержат магний и триптофан — мягкий вечерний ритуал при тревоге и для сна."}, {"id": "add4", "ch": "breakfast", "name": "Овсянка с черносливом и льном (для ЖКТ)", "time": 10, "tags": ["до 20 мин", "ЖКТ"], "kbju": {"k": 150, "p": 6, "f": 5, "c": 22}, "ing": ["Овсяные хлопья — 50г", "Молоко/вода — 200мл", "Чернослив — 4 шт", "Семена льна молотые — 1 ст.л.", "Грецкие орехи — 10г", "Корица — ½ ч.л."], "steps": ["Хлопья залить молоком, варить 5–7 мин.", "Чернослив нарезать, добавить в кашу.", "Сверху молотый лён, орехи, корица."], "masha": "~330 ккал, 11г белка. Утренний старт для регулярного ЖКТ.", "tip": "Молотый лён усваивается лучше целого.", "why": "Овёс (бета-глюкан) + чернослив (сорбитол) + молотый лён — три доказанных мягких помощника при запоре сразу."}];
  ...
  function IronSubTab() {
    ...
    const cols = [
      ...
      { id: "mid", l: "Середина", d: fmtD(KEY_DATES.analysisRemindFrom) },
      { id: "end", l: "Финиш", d: fmtD(KEY_DATES.planEnd) },
      ...extraCols,
    ];
  ...
  function PillStockTracker({ packAnchor }) {
    ...
    // Активные сегодня препараты (по ним и считаем остаток)
    let active = [];
    try { active = activePillsOn(today, packAnchor, pills); } catch { active = pills; }
  ...
  function IntakeGuide({ packAnchor }) {
    ...
    const RULES = {
      ...
      forlax: {
        emoji: "💧", label: "Форлакс",
        can: ["Развести в стакане воды и выпить", "Пить больше воды в течение дня"],
        cant: ["Принимать вплотную к другим таблеткам"],
        gapBefore: 1, gapAfter: 1, gapReason: "от других таблеток",
      ...
      biofemale: {
        emoji: "🦠", label: "Био Фимейл (пробиотик)",
        can: ["На ночь, запивать прохладной/тёплой водой"],
        cant: ["Не запивать горячим — убивает бактерии"],
        extra: "Поддержка микрофлоры.",
      },
      mel: {
  ...
  function TodayTab({ todayDow, DR, setTab, workoutDays, cycleAnchor, packAnchor, periodOverrides, badDay, userName }) {
    ...
    const _now = new Date();
    const analysisOn = _now >= KEY_DATES.analysisRemindFrom && _now <= KEY_DATES.block7Start && _ls("analysisRemindDismissed") !== "true";
  ...
  function CompatibilityMatrix() {
    const sections = [
      ...
      },
      {
        ...
        bg: C.oliveSoft,
        items: [
          { with: "Отдельно от таблеток", ok: false, note: "разрыв 1–2 ч — ускоряет транзит, может снижать всасывание других" },
      ...
      {
        name: "Цистениум / Био Фимейл",
        color: "#534AB7",
        bg: "#EEEDFE",
        items: [
          { with: "На ночь, с водой", ok: true, note: "совместимы со всем основным" },
          { with: "Био Фимейл — прохладная вода", ok: true, note: "горячая убивает культуры" },
        ]
      },
    ];
    ...
    return React.createElement("div", null,
      ...
      React.createElement("div", { style: { background: C.infoSoft, borderRadius: 9, padding: "10px 13px", border: `0.5px solid ${C.info}33`, marginBottom: 12 } },
        React.createElement("div", { style: { fontSize: 11, color: C.info, lineHeight: 1.6 } }, "💡 Омега, Био Фимейл, Ниацинамид, Мелатонин — совместимы со всем основным, можно в любое время.")
      ),
  ...
  function AnalysisReminder() {
    ...
    if (dismissed) return null;
    return React.createElement("div", {
      style: { padding: "10px 12px", background: C.bgWarm, borderLeft: `3px solid ${C.info}66`,
        borderRadius: 10, marginBottom: 8 }
    },
      React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 } },
        ...
        React.createElement("button", { onClick: () => setDismissed(true), "aria-label": "Скрыть",

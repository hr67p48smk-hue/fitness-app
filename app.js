(() => {
  ...
  function PillsModule({ compact, cycleAnchor, packAnchor }) {
    ...
    const markAllTaken = () => {
      const next = { ...taken };
      const at = { ...(taken.__at || {}) };
      activePills.forEach(p => { next[p.id] = true; if (at[p.id] == null) at[p.id] = smartTakeMin(p.id); });
      next.__at = at;
      setTaken(next);
    };
    ...
    // Время приёма: фактическое (__at.iron), иначе плановое время железа.
    let ironTimerText = null;
    if (taken.iron) {
      const ironPill = activePills.find(p => p.id === "iron");
      const ironAt = (taken.__at && taken.__at.iron != null) ? taken.__at.iron : (ironPill ? _pt(ironPill.time) : null);
      if (ironAt != null) {

(() => {
  ...
  // Якорь цикла по умолчанию: первый день ПОСЛЕДНИХ известных месячных.
  // У Маши: месячные были 11-15 мая 2026. Это якорь.
  // От него считаем: цикл 1 = 11 мая—7 июня (текущий), цикл 2 = 8 июня—5 июля и т.д.
  ...
  function getPeriodDay(date, anchorStr, overrides) {
    ...
    for (let n = Math.max(1, cycleNum - 1); n <= cycleNum + 1; n++) {
      ...
      const periodStart = ov ? mkd(ov) : getPredictedPeriodStart(n, anchorStr);
  ...
  function PillsModule({ compact, cycleAnchor, packAnchor }) {
    ...
    const DEFAULT_PILLS = [
      ...
      // 20:00 — A+E Medana (с ужином, с жирами)
      { id: "ae",         name: "A+E Medana (2 капсулы)", time: "20:00", color: EVENING, bg: EVENING_BG,
        note: "Витамин A+E (Vitaminum A+E Medana 2500 МЕ + 200 мг), 2 капсулы с ужином — жирорастворимые, нужна еда с жирами. Держать отдельно от витамина C и селена. Доза витамина А безопасная (~5000 МЕ/сут). Витамин E слегка разжижает кровь — учитывать вместе с Омегой и Яриной, сказать врачу перед операциями. С 8 июня.",
        startDate: "2026-06-08" },
    ...
    // V5 — добавлены A+E Medana и Цистениум (см. план). Версия поднята, чтобы догрузились новые препараты.
    const [pills, setPills] = useLS("pillsListV5", DEFAULT_PILLS);
  ...
  function CycleSubTab({ cycleAnchor: extAnchor, packAnchor: extPack }) {
    const [cycleAnchor, setCycleAnchor] = useLS("cycleAnchorV2", defaultCycleAnchor());
    ...
    const cycleAnchorDate = mkd(cycleAnchor);
    ...
    const cycles = [currentCycleNum, currentCycleNum + 1, currentCycleNum + 2, currentCycleNum + 3]
      ...
      .map(n => {
        ...
        const periodStart = ov ? mkd(ov) : start;
  ...
  function TodayTab({ todayDow, DR, setTab, workoutDays, cycleAnchor, packAnchor, periodOverrides, badDay, userName }) {
    ...
    let takenCount = 0;
    try {
      const stored = localStorage.getItem("pillsListV5");
  ...
  function OnboardingScreen({ onDone }) {
    ...
    // Когда юзер вводит дату первой таблетки пачки — предлагаем дату ПОСЛЕДНИХ
    // месячных (за 4 дня до неё). Это самый частый случай — месячные кончаются
    // и через 4 дня после последней таблетки начинается новая пачка.
    const onPackChange = (v) => {
      setPackAnchorVal(v);
      try {
        const d = mkd(v);
        // Последние месячные = за ~4 дня до начала пачки
        // (пачка стартует через 4 дня после начала перерыва = первый день месячных)
        d.setDate(d.getDate() - PERIOD_AFTER_LAST_PILL);
    ...
    const save = () => {
      try {
        ...
        localStorage.setItem("cycleAnchorV2", JSON.stringify(cycleAnchorVal));
    ...
    if (step === 1) {
      ...
      const lastPeriod = mkd(cycleAnchorVal);
      const lastPeriodEnd = new Date(lastPeriod); lastPeriodEnd.setDate(lastPeriodEnd.getDate() + PERIOD_LENGTH - 1);
      const nextPeriod = new Date(lastPill); nextPeriod.setDate(nextPeriod.getDate() + PERIOD_AFTER_LAST_PILL);
      const fmt = d => d.toLocaleDateString("ru-RU", { day: "numeric", month: "long" });

      return wrap(React.createElement("div", null,
        React.createElement("div", { style: { fontSize: 20, fontWeight: 600, color: C.text, marginBottom: 8 } }, "Ярина и цикл"),
        React.createElement("div", { style: { fontSize: 13, color: C.textM, lineHeight: 1.6, marginBottom: 18 } },
          "Когда была первая таблетка ", React.createElement("b", null, "текущей пачки"), "?"
        ),
        ...
        React.createElement("div", { style: { fontSize: 13, color: C.textM, lineHeight: 1.6, marginBottom: 8 } },
          "Когда были ", React.createElement("b", null, "последние месячные"), "? (первый день)"
        ),
        React.createElement("div", { style: { fontSize: 11, color: C.textL, marginBottom: 6 } },
          "Я предположила дату из пачки — поправь если другая"
        ),
        React.createElement("input", { type: "date", value: cycleAnchorVal, onChange: e => setCycleAnchorVal(e.target.value),
          style: { ...dateInputStyle, marginBottom: 16 }
        }),
        React.createElement("div", { style: { fontSize: 11, color: C.textL, lineHeight: 1.6, marginBottom: 18, padding: "10px 12px", background: C.bgWarm, borderRadius: 8 } },
          "📅 Последние месячные: ", React.createElement("b", { style: { color: C.pink } }, fmt(lastPeriod), "—", fmt(lastPeriodEnd)), React.createElement("br"),
          "💊 Последняя таблетка пачки: ", React.createElement("b", { style: { color: C.olive } }, fmt(lastPill)), React.createElement("br"),
          "🌸 Прогноз следующих месячных: ", React.createElement("b", { style: { color: C.pink } }, fmt(nextPeriod))
        ),
        React.createElement("div", { style: { display: "flex", gap: 8 } },
  ...
  const PLAN_BLOCKS = [
    ...
    },
    {
      ...
      sections: [
        { h: "Что подключается", items: [
          "С 8 июня — Gentle Iron 25 мг + витамин C в 11:30 (вт/чт/сб). Первый приём — ВТ 9 июня.",
  ...
  function PlanTab() {
    ...
    const [cycleAnchor] = useLS("cycleAnchorV2", defaultCycleAnchor());
